...
#include <linux/io.h>
//for AB partition BEGIN
#include <asm/setup.h>
//for AB partition END
 ...
 * Add for WCN MAC @{
 */
#define MAC_ADDR_MAXLEN    20
static char *wifimac_preload;
static int   wifimac_len = MAC_ADDR_MAXLEN;
static int   wifimac_open_times = 0;
static char  str_wifimac[MAC_ADDR_MAXLEN];

static char *btmac_preload;
static int   btmac_len = MAC_ADDR_MAXLEN;
static char  str_btmac[MAC_ADDR_MAXLEN];
/* @} */
...
static char fih_proc_test_result[FIH_PROC_SIZE] = {0};// RUNIN
...
struct st_fih_mem {
        ...
        unsigned int test_result;//test in RUNIN
        unsigned int tail;
};
...
static int hwmodel_show(struct seq_file *s, void *unused)
{
	...
	switch (hw_rfband)
	{
		case 0x00: strcat(str_hwmodel, "ROI"); break;
		case 0x01: strcat(str_hwmodel, "ROA"); break;
		case 0x02: strcat(str_hwmodel, "ROL"); break;
		case 0x03: strcat(str_hwmodel, "ROC"); break;
        case 0x04: strcat(str_hwmodel, "ROA"); break;
        case 0x05: strcat(str_hwmodel, "ROL"); break;	        
...
{
        if (!strcmp(fih_lcm_info, "ili9881h_hd720plus_dsi_vdo_dj"))
        {
                seq_printf(s, "ili9881h 5.9' 720P\n");
        }
        else if (!strcmp(fih_lcm_info, "hx83102_hd720plus_dsi_vdo_hlt"))
        {
        	seq_printf(s, "hx83102D 5.9' 720P\n");
           
        }
		else if (!strcmp(fih_lcm_info, "hx83102_hd720plus_dsi_vdo_truly"))
        {
        	seq_printf(s, "hx83102B 5.9' 720P\n");
...
//	printk("%s: %s\n", __func__, model[project_id].sim_num);
    ...
    if ((0x1 == project_id) || (0x2 == project_id)) {//for rooster
        if (0x3 == rf_id) {
            seq_printf(s, "%s\n", "vendor/etc/fqc_ds_ROC.xml");
            printk("!!!!vendor/etc/fqc_ds_ROC.xml\n");
        }
        else if (0x0 == rf_id) {
            seq_printf(s, "%s\n", "vendor/etc/fqc_ds_ROI.xml");
            printk("!!!!vendor/etc/fqc_ds_ROI.xml\n");
        }
        else if ((0x1 == project_id) && ((0x1 == rf_id) || (0x4 == rf_id))) {
            printk("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!fih_get_simslot=%d\n",fih_get_simslot());
            if (1 == fih_get_simslot()) {
                seq_printf(s, "%s\n", "vendor/etc/fqc_ss_ROA.xml");
                printk("!!!!vendor/etc/fqc_ss_ROA.xml\n");
            }
            else if (2 == fih_get_simslot()) {
                seq_printf(s, "%s\n", "vendor/etc/fqc_ds_ROA.xml");
                printk("!!!!vendor/etc/fqc_ds_ROA.xml\n");
            }
        }
        else if ((0x1 == project_id) && ((0x2 == rf_id) || (0x5 == rf_id))) {
            printk("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!fih_get_simslot=%d\n",fih_get_simslot());
            if (1 == fih_get_simslot()) {
                seq_printf(s, "%s\n", "vendor/etc/fqc_ss_ROL.xml");
                printk("!!!!vendor/etc/fqc_ss_ROL.xml\n");
            }
            else if (2 == fih_get_simslot()) {
                seq_printf(s, "%s\n", "vendor/etc/fqc_ds_ROL.xml");
                printk("!!!!vendor/etc/fqc_ds_ROL.xml\n");
            }
        }
        else if ((0x2 == project_id) && ((0x1 == rf_id) || (0x4 == rf_id))) {
            seq_printf(s, "%s\n", "vendor/etc/fqc_ss_ROA.xml");
            printk("!!!!vendor/etc/fqc_ss_ROA.xml\n");
        }
        else if ((0x2 == project_id) && ((0x2 == rf_id) || (0x5 == rf_id))) {
            seq_printf(s, "%s\n", "vendor/etc/fqc_ss_ROL.xml");
            printk("!!!!vendor/etc/fqc_ss_ROL.xml\n");
        }
        else {
...
//  if(!wifimac_open_times)
    if (1)
    {
        ...
        if(!IS_ERR(wifimac_filp))
        {
            ...
            seq_printf(s, "%02X:",(int)wifimac_preload[0]);
            seq_printf(s, "%02X:",(int)wifimac_preload[1]);
            seq_printf(s, "%02X:",(int)wifimac_preload[2]);
            seq_printf(s, "%02X:",(int)wifimac_preload[3]);
            seq_printf(s, "%02X:",(int)wifimac_preload[4]);
            seq_printf(s, "%02X",(int)wifimac_preload[5]);
        }
        else
        {
            printk("[WCN] Open %s fail\n", wifimac_BLOCK);
        }
...
static int bt_mac_show(struct seq_file *s, void *unused)
{
    ...
    if (1)
    {
        ...
        {
            printk("[dw]open %s fail=%ld\n", btmac_BLOCK, PTR_ERR(btmac_filp));
        }
