...
struct nRF24AP2_serial_hs_platform_data *private_data;
/*
...
static const struct of_device_id nRF24AP2_hs_match_table[] = {
	{ .compatible = "nordic,nrfap2-hsuart"},
	{}
};
...
static struct platform_driver nRF24AP2_serial_hs_platform_driver;
static struct uart_driver nRF24AP2_hs_driver;
static const struct uart_ops msm_hs_ops;
static void msm_hs_start_rx_locked(struct uart_port *uport);
static void msm_serial_hs_rx_work(struct kthread_work *work);
static void flip_insert_work(struct work_struct *work);
static void msm_hs_bus_voting(struct nRF24AP2_hs_port *msm_uport, unsigned int vote);
static struct nRF24AP2_hs_port *msm_hs_get_hs_port(int port_index);
static void msm_hs_queue_rx_desc(struct nRF24AP2_hs_port *msm_uport);
static int disconnect_rx_endpoint(struct nRF24AP2_hs_port *msm_uport);
static int msm_hs_pm_resume(struct device *dev);

#define UARTDM_TO_MSM(uart_port) \
	container_of((uart_port), struct nRF24AP2_hs_port, uport)

static int msm_hs_ioctl(struct uart_port *uport, unsigned int cmd,
						unsigned long arg)
{
	int ret = 0, state = 1;
	struct nRF24AP2_hs_port *msm_uport = UARTDM_TO_MSM(uport);
    ANT_FUNC_START();
	...
	case MSM_ENABLE_UART_CLOCK: {
		ret = nRF24AP2_hs_request_clock_on(&msm_uport->uport);
		break;
	}
	case MSM_DISABLE_UART_CLOCK: {
		ret = nRF24AP2_hs_request_clock_off(&msm_uport->uport);
		break;
	}
...
static int msm_hs_clk_bus_vote(struct nRF24AP2_hs_port *msm_uport)
{
	int rc = 0;
 ...
 */
static void msm_hs_clk_bus_unvote(struct nRF24AP2_hs_port *msm_uport)
{
...
static void msm_hs_resource_unvote(struct nRF24AP2_hs_port *msm_uport)
{
...
static void msm_hs_resource_vote(struct nRF24AP2_hs_port *msm_uport)
{
	int ret;
	struct uart_port *uport = &(msm_uport->uport);
...
static struct nRF24AP2_hs_port *get_matching_hs_port(struct platform_device *pdev)
{
	struct nRF24AP2_serial_hs_platform_data *pdata = pdev->dev.platform_data;
	struct nRF24AP2_hs_port *msm_uport = msm_hs_get_hs_port(pdev->id);
			  ...
			  char *buf)
{
	...
	struct platform_device *pdev = container_of(dev, struct
						    platform_device, dev);
	struct nRF24AP2_hs_port *msm_uport = get_matching_hs_port(pdev);
			 ...
			 const char *buf, size_t count)
{
	...
	if (msm_uport) {
		...
		case 0:
			...
			nRF24AP2_hs_request_clock_off(&msm_uport->uport);
			ret = count;
			break;
		case 1:
			MSM_HS_DBG("%s: Request clock ON\n", __func__);
			nRF24AP2_hs_request_clock_on(&msm_uport->uport);
			ret = count;
			break;
		default:
			ret = -EINVAL;
...
static inline bool is_use_low_power_wakeup(struct nRF24AP2_hs_port *msm_uport)
{
...
static void hex_dump_ipc(struct nRF24AP2_hs_port *msm_uport, void *ipc_ctx,
...

{
    ...
    PDBG("%s[0x%.10x:%d] : %s", prefix,(unsigned int)addr, size, buf);
    PDBG("%s  %s \n",prefix,buf);
}
 ...
 */
static void dump_uart_hs_registers(struct nRF24AP2_hs_port *msm_uport)
{
	struct uart_port *uport = &(msm_uport->uport);
...
{
	struct nRF24AP2_hs_port *msm_uport = data;
	struct uart_port *uport = &(msm_uport->uport);
	unsigned long flags;
	int ret = 0;
 ...
 */
static void msm_serial_debugfs_init(struct nRF24AP2_hs_port *msm_uport,
					   int id)
{
	char node_name[15];
...
static int nRF24AP2_hs_remove(struct platform_device *pdev)
{

	struct nRF24AP2_hs_port *msm_uport;
    struct nRF24AP2_serial_hs_platform_data *pdata = pdev->dev.platform_data;
	struct device *dev;
	...
	uart_remove_one_port(&nRF24AP2_hs_driver, &msm_uport->uport);
    ...
    /*gpio free*/
    if (gpio_is_valid(pdata->suspend_gpio))
        gpio_free(pdata->suspend_gpio);
    if (gpio_is_valid(pdata->sleep_gpio))
        gpio_free(pdata->sleep_gpio);
...
static int nRF24AP2_hs_spsconnect_tx(struct nRF24AP2_hs_port *msm_uport)
{
	int ret;
	struct uart_port *uport = &msm_uport->uport;
...
static int nRF24AP2_hs_spsconnect_rx(struct uart_port *uport)
{
	int ret;
	struct nRF24AP2_hs_port *msm_uport = UARTDM_TO_MSM(uport);
 ...
 */
unsigned int nRF24AP2_hs_tx_empty(struct uart_port *uport)
{
	unsigned int data;
	unsigned int isr;
	unsigned int ret = 0;
...
EXPORT_SYMBOL(nRF24AP2_hs_tx_empty);
...
static int sps_tx_disconnect(struct nRF24AP2_hs_port *msm_uport)
{
	struct uart_port *uport = &msm_uport->uport;
	struct msm_hs_tx *tx = &msm_uport->tx;
...
void nRF24AP2_tx_timeout_handler(unsigned long arg)
{
	struct nRF24AP2_hs_port *msm_uport = (struct nRF24AP2_hs_port *) arg;
	struct uart_port *uport = &msm_uport->uport;
	int isr;
    ANT_FUNC_START();
	if (msm_uport->pm_state != MSM_HS_PM_ACTIVE) {
...
static void msm_hs_post_rx_desc(struct nRF24AP2_hs_port *msm_uport, int inx)
{
...
static void msm_hs_mark_next(struct nRF24AP2_hs_port *msm_uport, int inx)
{
	struct msm_hs_rx *rx = &msm_uport->rx;
	int prev;
...
static void flip_insert_work(struct work_struct *work)
{
	...
	int retval;
	struct nRF24AP2_hs_port *msm_uport =
		container_of(work, struct nRF24AP2_hs_port,
...
static void msm_serial_hs_rx_work(struct kthread_work *work)
{
	...
	const struct nRF24AP2_serial_hs_platform_data *pdata;

    ANT_FUNC_START();
	msm_uport = container_of((struct kthread_work *) work,
				 struct nRF24AP2_hs_port, rx.kwork);
	msm_hs_resource_vote(msm_uport);
...
static void msm_serial_hs_tx_work(struct kthread_work *work)
{
	...
	struct nRF24AP2_hs_port *msm_uport =
			...
			struct nRF24AP2_hs_port, tx.kwork);
	struct uart_port *uport = &msm_uport->uport;
...
static void
msm_hs_mark_proc_rx_desc(struct nRF24AP2_hs_port *msm_uport,
			struct sps_event_notify *notify)
{
	struct msm_hs_rx *rx = &msm_uport->rx;
	phys_addr_t addr = DESC_FULL_ADDR(notify->data.transfer.iovec.flags,
 ...
 */
void nRF24AP2_hs_set_mctrl_locked(struct uart_port *uport,
				    unsigned int mctrl)
{
...
void nRF24AP2_hs_set_mctrl(struct uart_port *uport,
				    unsigned int mctrl)
{
	unsigned long flags;
	struct nRF24AP2_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	...
	msm_hs_resource_vote(msm_uport);
	spin_lock_irqsave(&uport->lock, flags);
	nRF24AP2_hs_set_mctrl_locked(uport, mctrl);
	spin_unlock_irqrestore(&uport->lock, flags);
	msm_hs_resource_unvote(msm_uport);
}
EXPORT_SYMBOL(nRF24AP2_hs_set_mctrl);
...
static irqreturn_t msm_hs_isr(int irq, void *dev)
{
	...
	struct nRF24AP2_hs_port *msm_uport = (struct nRF24AP2_hs_port *)dev;
	struct uart_port *uport = &msm_uport->uport;
	struct circ_buf *tx_buf = &uport->state->xmit;
	struct msm_hs_tx *tx = &msm_uport->tx;
 ...
 * port structure (struct uart_port or struct nRF24AP2_hs_port) given
 * the port index. msm_hs_get_uart port is called by clients.
 * The function msm_hs_get_hs_port is for internal use
 */

struct uart_port *nRF24AP2_hs_get_uart_port(int port_index)
{
	struct uart_state *state = nRF24AP2_hs_driver.state + port_index;
...
EXPORT_SYMBOL(nRF24AP2_hs_get_uart_port);

static struct nRF24AP2_hs_port *msm_hs_get_hs_port(int port_index)
{
	struct uart_port *uport = nRF24AP2_hs_get_uart_port(port_index);
...
void nRF24AP2_enable_wakeup_interrupt(struct nRF24AP2_hs_port *msm_uport)
{
	unsigned long flags;
	struct uart_port *uport = &(msm_uport->uport);
...
void nRF24AP2_disable_wakeup_interrupt(struct nRF24AP2_hs_port *msm_uport)
{
	unsigned long flags;
	struct uart_port *uport = &(msm_uport->uport);
...
void nRF24AP2_hs_resource_off(struct nRF24AP2_hs_port *msm_uport)
{
	struct uart_port *uport = &(msm_uport->uport);
	unsigned int data;
...
void nRF24AP2_hs_resource_on(struct nRF24AP2_hs_port *msm_uport)
{
	struct uart_port *uport = &(msm_uport->uport);
	unsigned int data;
	unsigned long flags;
	...
	nRF24AP2_hs_spsconnect_tx(msm_uport);
	if (msm_uport->rx.flush == FLUSH_SHUTDOWN) {
		nRF24AP2_hs_spsconnect_rx(uport);
		spin_lock_irqsave(&uport->lock, flags);
		msm_hs_start_rx_locked(uport);
		spin_unlock_irqrestore(&uport->lock, flags);
	}
...
int nRF24AP2_hs_request_clock_off(struct uart_port *uport)
{
	struct nRF24AP2_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	int ret = 0;
	int client_count = 0;
...
EXPORT_SYMBOL(nRF24AP2_hs_request_clock_off);

int nRF24AP2_hs_request_clock_on(struct uart_port *uport)
{
	struct nRF24AP2_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	int client_count;
	int ret = 0;
...
EXPORT_SYMBOL(nRF24AP2_hs_request_clock_on);
...
/**
 * nRF24AP2_hs_unconfig_uart_gpios: Unconfigures UART GPIOs
 * @uport: uart port
 */
static void nRF24AP2_hs_unconfig_uart_gpios(struct uart_port *uport)
{
...
/**
 * nRF24AP2_hs_config_uart_gpios - Configures UART GPIOs
 * @uport: uart port
 */
static int nRF24AP2_hs_config_uart_gpios(struct uart_port *uport)
{
	struct platform_device *pdev = to_platform_device(uport->dev);
	const struct nRF24AP2_serial_hs_platform_data *pdata =
					pdev->dev.platform_data;
...
static void nRF24AP2_hs_get_pinctrl_configs(struct uart_port *uport)
{
...
static int nRF24AP2_hs_startup(struct uart_port *uport)
{
	int ret;
	int rfr_level;
	unsigned long flags;
	...
	ret = nRF24AP2_hs_config_uart_gpios(uport);
	if (ret) {
	...
	/* Connect TX */
	sps_tx_disconnect(msm_uport);
	ret = nRF24AP2_hs_spsconnect_tx(msm_uport);
	if (ret) {
	...
	if (rx->flush != FLUSH_SHUTDOWN)
		disconnect_rx_endpoint(msm_uport);
	ret = nRF24AP2_hs_spsconnect_rx(uport);
	if (ret) {
...
unconfig_uart_gpios:
	nRF24AP2_hs_unconfig_uart_gpios(uport);
free_uart_irq:
...
struct nRF24AP2_serial_hs_platform_data
	*nRF24AP2_hs_dt_to_pdata(struct platform_device *pdev)
{
	struct device_node *node = pdev->dev.of_node;
	struct nRF24AP2_serial_hs_platform_data *pdata;
	u32 rx_to_inject;
    int ret;
    ANT_FUNC_START();
	...
	pdata->uart_tx_gpio = of_get_named_gpio(node,
					"fih_ant,ANT_UART_TX_MSM", 0);
	if (pdata->uart_tx_gpio < 0)
		PERROR("uart_tx_gpio is not available\n");

	/* UART RX GPIO */
	pdata->uart_rx_gpio = of_get_named_gpio(node,
					"fih_ant,ANT_UART_RX_MSM", 0);
	if (pdata->uart_rx_gpio < 0)
		PERROR("uart_rx_gpio is not available\n");

	/* UART CTS GPIO */
	pdata->uart_cts_gpio = of_get_named_gpio(node,
					"fih_ant,ANT_UART_RTS_MSM", 0);
	if (pdata->uart_cts_gpio < 0)
		PERROR("uart_cts_gpio is not available\n");

    /*GPIO */
    pdata->sleep_gpio = of_get_named_gpio(node, "fih_ant,ANT_GPIO_SLEEP", 0);
	if (gpio_is_valid(pdata->sleep_gpio)) {
		ret = gpio_request(pdata->sleep_gpio, "fih_ant,ANT_GPIO_SLEEP");
		if (ret) {
			PERROR("%s: unable to request ANT_GPIO_SLEEP gpio [%d]\n", __func__, pdata->sleep_gpio);
			goto request_gpio_fail;
		}
        gpio_direction_output(pdata->sleep_gpio, 0);
	}else
        PERROR("sleep_gpio is not available\n");

    pdata->suspend_gpio = of_get_named_gpio(node, "fih_ant,ANT_GPIO_SUSPEND", 0);
	if (gpio_is_valid(pdata->suspend_gpio)) {
		ret = gpio_request(pdata->suspend_gpio, "fih_ant,ANT_GPIO_SUSPEND");
		if (ret) {
			PERROR("%s: unable to request ANT_GPIO_SUSPEND gpio [%d]\n",__func__, pdata->suspend_gpio);
			goto sleep_gpio_unconfig;
		}
        gpio_direction_output(pdata->suspend_gpio, 1);
	}else
        PERROR("suspend_gpio is not available\n");
	...
	pdata->obs = of_property_read_bool(node,
				"qcom,msm-obs");
	if (pdata->obs)
		PERROR("%s:Out of Band sleep flag is set\n", __func__);
	...
	if (pdata->inject_rx_on_wakeup) {
		ret = of_property_read_u32(node, "qcom,rx-char-to-inject",
						&rx_to_inject);
		if (ret < 0) {
			PERROR("Error: Rx_char_to_inject not specified.\n");
			return ERR_PTR(ret);
		}
	...
	if (ret < 0) {
		PERROR("Error: Getting UART BAM TX EP Pipe Index.\n");
		return ERR_PTR(ret);
	}

	if (!(pdata->bam_tx_ep_pipe_index >= BAM_PIPE_MIN &&
		pdata->bam_tx_ep_pipe_index <= BAM_PIPE_MAX)) {
		PERROR("Error: Invalid UART BAM TX EP Pipe Index.\n");
 ...
 * @msm_uport - Pointer to nRF24AP2_hs_port structure
 * @ep - Pointer to sps endpoint data structure
 */

static void msm_hs_exit_ep_conn(struct nRF24AP2_hs_port *msm_uport,
...
static int msm_hs_sps_init_ep_conn(struct nRF24AP2_hs_port *msm_uport,
				struct msm_hs_sps_ep_conn_data *ep,
				bool is_producer)
{
	int rc = 0;
...
static int msm_hs_sps_init(struct nRF24AP2_hs_port *msm_uport)
{
	int rc = 0;
 ...
 * used, the nRF24AP2_hs_dt_to_pdata gets it and the boolean array
...
static void obs_manage_irq(struct nRF24AP2_hs_port *msm_uport, bool en)
{
	struct uart_port *uport = &(msm_uport->uport);
...
static void pm_init(struct nRF24AP2_serial_hs_platform_data *pdata)
{
    PINFO("Enter pm_init \n");

    //1. Enter suspend mode, SLEEP must be raised before ~SUSPEND is asserted
    gpio_set_value(pdata->sleep_gpio, 1); // High active GPIO, 1 enable , 0 disable
    msleep(SLEEP_DELAY);
    PDBG("set sleep GPIO to 1\n");

    //2. set SUSPEN to 0 to enter suspend mode.
    gpio_set_value(pdata->suspend_gpio, 0);// Low active GPIO, 0 enable, 1 disable
    msleep(SLEEP_DELAY);
    PDBG("set suspend GPIO to 0\n");  
    return;
}
...
static void msm_hs_pm_suspend(struct device *dev)
{
	...
	nRF24AP2_hs_resource_off(msm_uport);
	...
	if (!atomic_read(&msm_uport->client_req_state))
		nRF24AP2_enable_wakeup_interrupt(msm_uport);
	LOG_USR_MSG(msm_uport->ipc_msm_hs_pwr_ctxt,
		"%s: PM State Suspended client_count %d\n", __func__,
								client_count);
	mutex_unlock(&msm_uport->mtx);
    PINFO("Enter suspend mode\n");
    //1. Enter suspend mode, SLEEP must be raised before ~SUSPEND is asserted
    gpio_set_value(pdata->sleep_gpio, 1); // High active GPIO, 1 enable , 0 disable
    msleep(SLEEP_DELAY);
    PDBG("set sleep GPIO to 1\n");

    //2. set SUSPEN to 1 to enter suspend mode.
    gpio_set_value(pdata->suspend_gpio, 0);// Low active GPIO, 0 enable, 1 disable
    msleep(SLEEP_DELAY);
    PDBG("set suspend GPIO to 0\n");    
    ANT_FUNC_END();
...
static int msm_hs_pm_resume(struct device *dev)
{
	...
	if (msm_uport->pm_state == MSM_HS_PM_ACTIVE)
		goto exit_pm_resume;
	if (!atomic_read(&msm_uport->client_req_state))
		nRF24AP2_disable_wakeup_interrupt(msm_uport);
	...
	nRF24AP2_hs_resource_on(msm_uport);

	LOG_USR_MSG(msm_uport->ipc_msm_hs_pwr_ctxt,
		"%s:PM State:Active client_count %d\n", __func__, client_count);
    
    PINFO("Enter resume mode\n");

    //1. exit suspend mode
    gpio_set_value(pdata->suspend_gpio, 1);// Low active GPIO, 0 enable, 1 disable    
    msleep(SLEEP_DELAY);
	PDBG("set suspend GPIO to 1\n");

    //2. set SUSPEN to 1 to enter suspend mode.
    gpio_set_value(pdata->sleep_gpio, 0); // High active GPIO, 1 enable , 0 disable
    msleep(SLEEP_DELAY);
	PDBG("set sleep GPIO to 0\n");
    ANT_FUNC_END();
...
int nRF24AP2_ant_power_on(struct nRF24AP2_serial_hs_platform_data *pdata, bool on)
{
    int retval;
    ANT_FUNC_START();
    ...
    if (on){
        if(pdata->vcc_ana){
            ...
            if (retval){
                PERROR("Failed to enable regulator avdd\n");
                return retval;
            }
        }
    }else{
...
static int  nRF24AP2_ant_regulator_configure(struct platform_device *pdev, struct nRF24AP2_serial_hs_platform_data *pdata)
{
    int ret=0;    
    
    pdata->vcc_ana = regulator_get(&pdev->dev, "vcc_ana");
    if (IS_ERR(pdata->vcc_ana)) {        
        PERROR("pdata->vcc_ana fail\n");
        return -1;
    }

    if (regulator_count_voltages(pdata->vcc_ana) > 0) {        
        ret = regulator_set_voltage(pdata->vcc_ana, 3000000, 3000000);
        PINFO("set Vcc voltage 3V.");
    }
    return ret;
}

static int nRF24AP2_hs_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct uart_port *uport;
	struct nRF24AP2_hs_port *msm_uport;
	...
	if (pdev->dev.of_node) {
		...
		pdata = nRF24AP2_hs_dt_to_pdata(pdev);
		if (IS_ERR(pdata))
    ...
    //power supply    
    ret = nRF24AP2_ant_regulator_configure(pdev,pdata);
    if (ret < 0 ){
        PERROR("Failed to set voltage configure, ret=%d \n",ret);
        return ret;
    } 
    
    nRF24AP2_ant_power_on(pdata,true);
	...
	msm_uport = devm_kzalloc(&pdev->dev, sizeof(struct nRF24AP2_hs_port),
			GFP_KERNEL);
	if (!msm_uport)
		return -ENOMEM;
	...
	nRF24AP2_hs_get_pinctrl_configs(uport);
	...
	ret = uart_add_one_port(&nRF24AP2_hs_driver, uport);
	if (!ret) {
		msm_hs_clk_bus_unvote(msm_uport);
...
static int __init nRF24AP2_serial_hs_init(void)
{
	int ret;

    ANT_FUNC_START();
	ret = uart_register_driver(&nRF24AP2_hs_driver);
	if (unlikely(ret)) {
	...
	ret = platform_driver_register(&nRF24AP2_serial_hs_platform_driver);
	if (ret) {
		pr_err("%s failed to load\n", __func__);
		debugfs_remove_recursive(debug_base);
		uart_unregister_driver(&nRF24AP2_hs_driver);
		return ret;
	}
 ...
 *     - Unhook the ISR
 */
static void nRF24AP2_hs_shutdown(struct uart_port *uport)
{
	int ret, rc;
	struct nRF24AP2_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	struct circ_buf *tx_buf = &uport->state->xmit;
    ...
    PINFO("%s:UART port closed successfully\n", __func__);
    ANT_FUNC_END();
}

static void __exit nRF24AP2_serial_hs_exit(void)
{
	pr_info("msm_serial_hs module removed\n");
	debugfs_remove_recursive(debug_base);
	platform_driver_unregister(&nRF24AP2_serial_hs_platform_driver);
	uart_unregister_driver(&nRF24AP2_hs_driver);
}

static const struct dev_pm_ops nRF24AP2_hs_dev_pm_ops = {
...
static struct platform_driver nRF24AP2_serial_hs_platform_driver = {
	.probe	= nRF24AP2_hs_probe,
	.remove = nRF24AP2_hs_remove,
	.driver = {
		.name = DRIVER_NAME,
		.pm   = &nRF24AP2_hs_dev_pm_ops,
		.of_match_table = nRF24AP2_hs_match_table,
	},
};
...
static const struct uart_ops msm_hs_ops = {
	.tx_empty = nRF24AP2_hs_tx_empty,/* 串口的Tx FIFO緩存是否為空 */
	.set_mctrl = nRF24AP2_hs_set_mctrl_locked,/* 設置串口modem控制 */
	.get_mctrl = msm_hs_get_mctrl_locked, /* 獲取串口modem控制 */
	.stop_tx = msm_hs_stop_tx_locked,/* 禁止串口發送數據 */
	.start_tx = msm_hs_start_tx_locked, /* 使能串口發送數據 */
	.stop_rx = msm_hs_stop_rx_locked, /* 禁止串口接收數據 */
	.enable_ms = msm_hs_enable_ms_locked,/* 使能modem的狀態信號 */
	.break_ctl = msm_hs_break_ctl,/* 設置break信號 */
	.startup = nRF24AP2_hs_startup,/* ?動串口,應用程序打開串口設備文件時,該函數會被調用 */
	.shutdown = nRF24AP2_hs_shutdown, /* 關閉串口,應用程序關閉串口設備文件時,該函數會被調用 */
	.set_termios = msm_hs_set_termios,/* 設置串口參數 */
	.type = msm_hs_type,
	.config_port = msm_hs_config_port,/* 執行串口所需的自動配置 */
	.flush_buffer = NULL,
	.ioctl = msm_hs_ioctl,
};

module_init(nRF24AP2_serial_hs_init);
module_exit(nRF24AP2_serial_hs_exit);
MODULE_DESCRIPTION("High Speed UART Driver for NRFAP2 ANT+");
