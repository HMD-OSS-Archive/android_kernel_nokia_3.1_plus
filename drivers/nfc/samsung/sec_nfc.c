 ...
 * Porting SEC NFC kernel driver @{
 */
#include <linux/i2c.h>
#include <linux/of_platform.h>
#include <linux/dma-mapping.h>
#include <mt-plat/upmu_common.h>        /* pmic & ptim */
#include <mt-plat/mtk_boot_common.h>    /* boot mode*/

#define __GPIO_DEVRES_API    (1)
#define __GPIO_CANSLEEP_API  (1)
/* @}*/
...
/* J6000504 - roy.zy.shang
 * To detect whether the NFC chip is attached or not.
 */
static int sec_nfc_chip_exist = 0;
...
int sec_nfc_i2c_probe(struct i2c_client *client)
{
        ...
        dev_dbg(info->dev, "%s: start: %p\n", __func__, info);
        //printk(KERN_INFO "[NFC] info->dev, "%s: start: %p\n", __func__, info);
        ...
        /* J6000504 - roy.zy.shang : using devres API*/
#ifndef __GPIO_DEVRES_API
...
static int sec_nfc_suspend(struct device *dev)
{
        ...
        int ret = 0;
        if(sec_nfc_chip_exist == 0){
                return 0;
        }
        info = SEC_NFC_GET_INFO(dev);
        mutex_lock(&info->mutex);
...
static int __sec_nfc_probe(struct device *dev)
{
        ...
        //printk(KERN_INFO "[NFC] __sec_nfc_probe\n");
...
/* J6000504 - roy.zy.shang : using devres API*/
#ifndef __GPIO_DEVRES_API
        ret = gpio_request(pdata->avdd, "nfc_pvdd");
#else
        ret = devm_gpio_request_one(dev, pdata->avdd, GPIOF_OUT_INIT_LOW, "nfc_pvdd");
#endif
        if (ret) {
                dev_err(dev, "failed to get gpio pvdd\n");
                goto err_gpio_pvdd;
        }
#ifdef __GPIO_CANSLEEP_API
        if(get_boot_mode() != KERNEL_POWER_OFF_CHARGING_BOOT) {
                gpio_set_value_cansleep(pdata->avdd, 1);
        }
#else
        gpio_direction_output(pdata->avdd, 1);
#endif
        dev_dbg(dev, "[NFC] pvdd-gpio 178 = %d\n", gpio_get_value(pdata->avdd));
...
static int __sec_nfc_remove(struct device *dev)
{
        ...
        dev_dbg(info->dev, "%s\n", __func__);
        //printk("__sec_nfc_remove: remove the NFC device\n");
...
{
        int ret = 0;
        //printk(KERN_INFO "[NFC] sec_nfc_probe\n");
...
static void __sec_nfc_shutdown(struct device *dev)
{
        struct sec_nfc_info *info = dev_get_drvdata(dev);
        //struct i2c_client *client = info->i2c_info.i2c_dev;
        struct sec_nfc_platform_data *pdata = info->pdata;
        ...
        dev_err(info->dev, "%s\n", __func__);
        //printk(KERN_INFO "[NFC] __sec_nfc_shutdown: pvdd-gpio 178 = %d\n", gpio_get_value(pdata->avdd));
        gpio_set_value(pdata->avdd, 0);
        if (pdata->avdd) gpio_free(pdata->avdd);
...
/****
        ...
        gpio_direction_output(pdata->avdd, 0);

        dev_err(dev, "[NFC] pvdd-gpio 178 = %d\n", gpio_get_value(pdata->avdd));
**/
    //printk(KERN_INFO "[NFC] sec_nfc_shutdown\n");
    if(sec_nfc_chip_exist == 0){
        return;
    }

    __sec_nfc_shutdown(&client->dev);
...
{
        //printk(KERN_INFO "[NFC] sec_nfc_init\n");
