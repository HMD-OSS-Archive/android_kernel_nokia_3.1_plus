...
unsigned char* fih_3l6_awbaf_data;
unsigned char* fih_3l6_lsc_data;
unsigned char* fih_bokeh_param;
unsigned char* fih_4h7_lsc_data;
unsigned char fih_5e9_vendor_id;

//#define NEW_UNLOCK_IOCTL
//#ifndef NEW_UNLOCK_IOCTL
//static int EEPROM_drv_ioctl(struct inode *a_pstInode, struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
//#else
static long EEPROM_drv_ioctl(struct file *file, unsigned int a_u4Command, unsigned long a_u4Param)
//#endif
{
	int i4RetValue = 0;
	u8 *pBuff = NULL;
	...
	case CAM_CALIOC_G_READ:
		printk("jasondz: CAM_CALIOC_G_READ start! offset=0x%x, length=0x%x, sensorid 0x%x, deviceid %d\n",
			ptempbuf->u4Offset, ptempbuf->u4Length, ptempbuf->sensorID, ptempbuf->deviceID);
        ...
        if (pcmdInf == NULL || pcmdInf->readCMDFunc == NULL) {
            PK_DBG("pcmdInf get Error!\n");
            kfree(pBuff);
            kfree(pu1Params);
            return -EFAULT;
        }

        if (ptempbuf->deviceID == 1) 
        {
            if (fih_3l6_awbaf_data == NULL || fih_3l6_lsc_data == NULL || fih_bokeh_param == NULL) 
            {
                printk("jasondz: 3l6 buffer fail, read through i2c\n");
                i4RetValue = pcmdInf->readCMDFunc(pcmdInf->client, ptempbuf->u4Offset, pu1Params, ptempbuf->u4Length);
            }
            else
            {
                i4RetValue = ptempbuf->u4Length;

                if (ptempbuf->u4Offset == 0x0a) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 0, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x13) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 9, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x14) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 10, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x1a) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 16, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x1e) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 20, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x22) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 24, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x25) 
                    memcpy(pu1Params, fih_3l6_lsc_data, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x27) 
                    memcpy(pu1Params, fih_3l6_lsc_data+2, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x774) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 25, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x776) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 27, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x778) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 29, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x77a) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 31, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x77c) 
                    memcpy(pu1Params, fih_3l6_awbaf_data + 33, ptempbuf->u4Length);
                else if (ptempbuf->u4Offset == 0x1016) 
                    memcpy(pu1Params, fih_bokeh_param, ptempbuf->u4Length);   
                else 
                    i4RetValue = pcmdInf->readCMDFunc(pcmdInf->client, ptempbuf->u4Offset, pu1Params, ptempbuf->u4Length);
            }
        }
        else if (ptempbuf->deviceID == 2) 
        {
            if (fih_4h7_lsc_data == NULL) 
            {
                printk("jasondz: 4h7 buffer fail, read through i2c\n");
                i4RetValue = pcmdInf->readCMDFunc(pcmdInf->client, ptempbuf->u4Offset, pu1Params, ptempbuf->u4Length);
            }
            else
            {
                ...
                if (fih_4h7_lsc_data[1] == 0x1E) 
                {
                    printk("jasondz: 4h7 lce source!\n");
                    if (ptempbuf->u4Offset == 0x0a) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 13, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x13) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 17, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x14) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 2, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x1a) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 33, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x1e) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 37, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x22) 
                    {
                        memcpy(pu1Params, fih_4h7_lsc_data + 44, ptempbuf->u4Length);
                        *pu1Params -= 1;
                    }
                    else if (ptempbuf->u4Offset == 0x25) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 65, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x27) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 67, ptempbuf->u4Length); 
                    else 
                        i4RetValue = pcmdInf->readCMDFunc(pcmdInf->client, ptempbuf->u4Offset, pu1Params, ptempbuf->u4Length);
                }
                else
                {
                    printk("jasondz: 4h7 truly source!\n");
                    if (ptempbuf->u4Offset == 0x0a) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 10, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x13) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 19, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x14) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 20, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x1a) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 26, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x1e) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 30, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x22) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 34, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x25) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 65, ptempbuf->u4Length);
                    else if (ptempbuf->u4Offset == 0x27) 
                        memcpy(pu1Params, fih_4h7_lsc_data + 67, ptempbuf->u4Length); 
        ...
        else if (ptempbuf->deviceID == 4) 
        {
            if (ptempbuf->u4Offset == 0x1c) 
            {
                if (fih_5e9_vendor_id == 2) 
                    i4RetValue = pcmdInf->readCMDFunc(pcmdInf->client, ptempbuf->u4Offset, pu1Params, ptempbuf->u4Length);
                else
                    i4RetValue = pcmdInf->readCMDFunc(pcmdInf->client, ptempbuf->u4Offset-2, pu1Params, ptempbuf->u4Length);
            }
            else 
                i4RetValue = pcmdInf->readCMDFunc(pcmdInf->client, ptempbuf->u4Offset, pu1Params, ptempbuf->u4Length);
        }
		...
		printk("jasondz: Read data %d bytes take %lu us\n", ptempbuf->u4Length, TimeIntervalUS);
#endif
