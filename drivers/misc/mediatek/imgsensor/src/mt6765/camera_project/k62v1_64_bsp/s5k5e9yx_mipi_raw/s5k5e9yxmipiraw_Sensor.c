...
static imgsensor_info_struct imgsensor_info = { 
	...
	.cap1 = {
		.pclk = 190000000,				/* record different mode's pclk */
		.linelength  = 3112,				/* record different mode's linelength */
		.framelength = 4068,			/* record different mode's framelength */
		.startx = 0,					/* record different mode's startx of grabwindow */
		.starty = 0,					/* record different mode's starty of grabwindow */
		.grabwindow_width  = 2608,		/* record different mode's width of grabwindow */
		.grabwindow_height = 1960,		/* record different mode's height of grabwindow */
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) 
            {
                ...
                fih_read_eeprom_data(0x4, 1, &mid, 0xA2);
                printk("jasondz: 5e9 read out mid %d, wanted truly mid 2\n", mid);
                if (mid != 2) 
                {
				...
				LOG_INF("5e9 truly jasondz: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			LOG_INF("5e9 truly jasondz: Read sensor id fail,i2c_write_id 0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
