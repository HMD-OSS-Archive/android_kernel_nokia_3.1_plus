...
//[zcgadd] 20180723 add vcamaf power control function to support VCM work start[
/**********************************************************************/
static void switchAFON(void)
{
   atomic_set(&zcgcnt, 1);
}
static void switchAFOFF(void)
{
    atomic_set(&zcgcnt, 0);
}
static int checkAFStatus(void)
{
int ret =0;

ret = atomic_read(&zcgcnt);
    if (ret > 0) {
        pr_debug("[zcgregulator] Now VCAMAF Status is ON %d\n", ret);
     }
    else{
        pr_debug("[zcgregulator]  Now VCAMAF Status is OFF %d\n", ret);
    }
  return ret;

}
int myctrlAFPower(int onoff)
{
    int ret = -1;

    struct device* p_device;
    struct regulator*  p_reg_af;

   p_device  = gimgsensor_device;
   
   p_device->of_node = of_find_compatible_node(NULL, NULL, "mediatek,camera_hw");

   if (p_device->of_node == NULL) 
    {
		pr_err("[zcgregulator] regulator get cust camera node failed!\n");
		return IMGSENSOR_RETURN_ERROR;
	}

   p_reg_af = regulator_get(p_device, "vcamaf");

   if (onoff == 1) 
    {
       ret= regulator_set_voltage( p_reg_af, 2800000, 2800000);
       ret = regulator_enable(p_reg_af);
   ...
   else
    {
       ...
       if(regulator_is_enabled(p_reg_af))
	   {
            ret = regulator_disable(p_reg_af);
            
			if (ret)
               {
				 pr_err("[zcgregulator]fail to regulator_disable1\n");
				 return IMGSENSOR_RETURN_ERROR;
				}
	   }

    }
  pr_err("[zcgregulator] check af status %d\n", checkAFStatus());
   return ret;
}
/************************************************************************/
//[zcgadd] 20180723 add vcamaf power control function to support VCM work end]
static enum IMGSENSOR_RETURN regulator_init(void *pinstance)
{
	...
	for (i = 0; i < REGULATOR_TYPE_MAX_NUM; i++, pregulator_ctrl++) {
		...
		if (preg->pregulator[i] == NULL)
			pr_err("jasondz: regulator_init regulator[%d]  %s fail!\n",
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	enable_cnt =
        ...
        pr_err(
        "jasondz regulator_set, sensoridx %d powertype:%d powerId:%d\n",
        sensor_idx, pin,
