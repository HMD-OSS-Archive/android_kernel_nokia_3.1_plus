...
extern unsigned char* fih_4h7_lsc_data;
...
static int mc_write(struct seq_file *s, void *unused)
{
	if (fih_4h7_lsc_data[1] == 0x1E)
		seq_printf(s,"Samsung S5K4H7, Vendor: LCE, VendorID %d\n", fih_4h7_lsc_data[1]);
	else
		seq_printf(s,"Samsung S5K4H7, Vendor: Truly, VendorID %d\n", fih_4h7_lsc_data[4]);
...
static void sensor_init(void)
{
	...
	//jasondz add for hw signal test, Tr/Tf Control adjust
...
static void preview_setting(void)
{
	...
	write_cmos_sensor_8(0x3C17, 0x01);
...
static void fih_4h7_get_lsc_data(void)
{
    ...
    printk("jasondz2: fih_4h7_get_lsc_data start\n");

	write_cmos_sensor_8(0x0A00,0x03);
    ...
    get_byte = read_cmos_sensor(0x0A04);
    if (get_byte != 1) {
        printk("jasondz2: awb group 1 is invalid, flag is %d, change to group 2\n", get_byte);
        write_cmos_sensor_8(0x0A00,0x03);
        ...
        get_byte = read_cmos_sensor(0x0A04);
        printk("jasondz2: awb group 2, flag is %d\n", get_byte);
    }

    fih_read_eeprom_data(0x0A04, 64, fih_4h7_lsc_data, 0x20);

	for(j = 0; j < 64; j++){
        printk("jasondz2: %x, %x, %x, %x, %x, %x, %x, %x, \n", 
               fih_4h7_lsc_data[j], fih_4h7_lsc_data[j+1], 
               fih_4h7_lsc_data[j+2], fih_4h7_lsc_data[j+3], 
               fih_4h7_lsc_data[j+4], fih_4h7_lsc_data[j+5], 
               fih_4h7_lsc_data[j+6], fih_4h7_lsc_data[j+7]);
        j += 7;
    }

    // check which lsc group is valid
	write_cmos_sensor_8(0x0A00,0x03);
	write_cmos_sensor_8(0x0A02,23);
	write_cmos_sensor_8(0x0A00,0x01);
    ...
    if (get_byte == 1) {
        Page = 23;
        printk("jasondz2: fih_4h7_get_lsc_data group 1 is valid, page = %d\n", Page);
    }
    else 
    {
    	write_cmos_sensor_8(0x0A00,0x03);
    	write_cmos_sensor_8(0x0A02,53);
    	write_cmos_sensor_8(0x0A00,0x01);
        ...
        if (get_byte == 1) {
            Page = 53;
            printk("jasondz2: fih_4h7_get_lsc_data group 2 is valid, page = %d\n", Page);
        }
    }

    if (Page != 23 && Page != 53) {
        printk("jasondz2: fih_4h7_get_lsc_data NO group is valid, return!\n");
        return;
    }
	...
	for(j = 0; j < 30; j++){
        ...
        fih_read_eeprom_data(0x0A04, 64, fih_4h7_lsc_data+i, 0x20);

        Page++; i += 64;
	}

	for(j = 65; j < 1935; j++){
        check_sum += fih_4h7_lsc_data[j];
    }

    printk("jasondz2: fih_4h7_get_lsc_data check sum %d, reg %d\n", check_sum%255, fih_4h7_lsc_data[1935]);

    printk("jasondz2: fih_4h7_get_lsc_data end\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			*sensor_id = return_sensor_id();
            ...
            {
				printk("jasondz: 4h7 get id ok! ReadOut sensor id: 0x%x, Wanted sensor id:0x%x\n", *sensor_id, imgsensor_info.sensor_id);

                fih_4h7_lsc_data = kmalloc(1984, GFP_KERNEL);
                if (fih_4h7_lsc_data != NULL) {
                    fih_4h7_get_lsc_data();
                }
            ...
            printk("jasondz: 4h7 get id fail! ReadOut sensor id: 0x%x, Wanted sensor id:0x%x\n", *sensor_id, imgsensor_info.sensor_id);
			retry--;
		} while(retry > 0);
