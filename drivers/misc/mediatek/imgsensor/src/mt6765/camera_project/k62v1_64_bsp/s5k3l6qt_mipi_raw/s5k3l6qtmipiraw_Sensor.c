...
#include "s5k3l6qtmipiraw_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = {
	...
	.cap1 = {
		.pclk = 480000000,				/* record different mode's pclk */
		.linelength  = 4896,				/* record different mode's linelength */
		.framelength = 4060,			/* record different mode's framelength */
		.startx = 0,					/* record different mode's startx of grabwindow */
		.starty = 0,					/* record different mode's starty of grabwindow */
		.grabwindow_width  = 4208,		/* record different mode's width of grabwindow */
		.grabwindow_height = 3120,		/* record different mode's height of grabwindow */
...
{
    seq_printf(s,"Samsung S5K3L6+5E9, Vendor: QTech, VendorID %d\n", mid);
...
#if MULTI_WRITE
static kal_uint16 addr_data_pair_capture_s5k3l6[] = {
	0x0344, 0x0008,
	0x0346, 0x0008,
	0x0348, 0x1077,
	0x034A, 0x0C37,
...
extern unsigned char* fih_3l6_awbaf_data;
extern unsigned char* fih_3l6_lsc_data;
extern unsigned char* fih_bokeh_param;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id()+1;
            ...
            {
                fih_read_eeprom_data(0x4, 1, &mid, 0xA0);

                if (mid != 4) 
                {
                    printk("jasondz: 3l6 read out mid %d, wanted qtech mid 4, return connect fail!\n", mid);
                    *sensor_id = 0xFFFFFFFF;
                    return ERROR_SENSOR_CONNECT_FAIL;
                }

                fih_3l6_awbaf_data = kmalloc(36, GFP_KERNEL);
                ...
                fih_3l6_lsc_data = kmalloc(1870, GFP_KERNEL);
                if (fih_3l6_lsc_data != NULL) {
                    fih_read_eeprom_data(0x23, 1870, (u8 *)fih_3l6_lsc_data, 0xA0);
                }

                fih_bokeh_param = kmalloc(2048, GFP_KERNEL);
        		if (fih_bokeh_param != NULL) {
                    fih_read_eeprom_data(0x1016, 2048, (u8 *)fih_bokeh_param, 0xA0);
        		}
				...
				printk("jasondz: 3l6 qtech get id ok! ReadOut sensor id: 0x%x, Wanted sensor id:0x%x\n", *sensor_id, imgsensor_info.sensor_id);

				return ERROR_NONE;
			}

            printk("jasondz: 3l6 qtech get id fail! ReadOut sensor id: 0x%x, Wanted sensor id:0x%x\n", *sensor_id, imgsensor_info.sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 1;
