...
#define PFX "s5k5e9qt_camera_sensor"
#define LOG_1 LOG_INF("s5k5e9qt,MIPI 2LANE\n")
...
*
*************************************************************************/
extern unsigned char fih_5e9_vendor_id;
		...
		do {
			*sensor_id = return_sensor_id()+1;
			if (*sensor_id == imgsensor_info.sensor_id) 
            {
				...
				printk("jasondz: 5e9 read out mid %d, wanted qtech mid 4\n", mid);
                if (mid != 4) 
                {
                ...
                fih_5e9_vendor_id = mid;

				LOG_INF("5e9 qtech jasondz: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			LOG_INF("5e9 qtech jasondz: Read sensor id fail,i2c_write_id 0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
UINT32 S5K5E9QT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
