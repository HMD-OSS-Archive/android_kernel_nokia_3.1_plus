...
static enum IMGSENSOR_RETURN imgsensor_hw_power_sequence_sensor(
	struct IMGSENSOR_HW             *phw,
	enum   IMGSENSOR_SENSOR_IDX      sensor_idx,
	enum   IMGSENSOR_HW_POWER_STATUS pwr_status,
	struct IMGSENSOR_HW_POWER_SEQ_SENSOR   *ppower_sequence,
	char *pcurr_idx)
{
	struct IMGSENSOR_HW_SENSOR_POWER *psensor_pwr =
	    &phw->sensor_pwr[sensor_idx];

	struct IMGSENSOR_HW_POWER_SEQ_SENSOR    *ppwr_seq = ppower_sequence;
	struct IMGSENSOR_HW_POWER_INFO_SENSOR   *ppwr_info;
	struct IMGSENSOR_HW_DEVICE       *pdev;
	int                               pin_cnt = 0;
	...
	enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	imgsensor_hw_power_sequence_sensor(
	    phw,
	    sensor_idx,
	    pwr_status,
