...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
    ...
    if (!strcmp(psensor_inst->psensor_name,SENSOR_DRVNAME_S5K3L6_MIPI_RAW) || !strcmp(psensor_inst->psensor_name,SENSOR_DRVNAME_S5K3L6QT_MIPI_RAW)) 
    {
        if (psensor_inst->sensor_idx != 0) 
        {
            printk("jasondz id: %d, name %s, mismatch skip\n", psensor_inst->sensor_idx, psensor_inst->psensor_name);
            return -EIO;
        }
    }
    else if (psensor_inst->sensor_idx == 1 && strcmp(psensor_inst->psensor_name,SENSOR_DRVNAME_S5K4H7YX_MIPI_RAW)) {
        printk("jasondz id: %d, name %s, mismatch skip\n", psensor_inst->sensor_idx, psensor_inst->psensor_name);
        return -EIO;
    }
    else if (!strcmp(psensor_inst->psensor_name,SENSOR_DRVNAME_S5K5E9YX_MIPI_RAW) || !strcmp(psensor_inst->psensor_name,SENSOR_DRVNAME_S5K5E9QT_MIPI_RAW)) 
    {
        if (psensor_inst->sensor_idx != 2) 
        {
            printk("jasondz id: %d, name %s, mismatch skip\n", psensor_inst->sensor_idx, psensor_inst->psensor_name);
            return -EIO;
        }
...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	pr_err("wangrui get_search_list %d,\n %d %d %d %d\n %d %d %d %d\n",
	    get_search_list,
	    orderedSearchList[0],
	    orderedSearchList[1],
	    orderedSearchList[2],
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	sizeof(mtk_ccm_name),
				"%s\nCAM[%d]:%s;",
				mtk_ccm_name, pSensorGetInfo->SensorId, psensor->inst.psensor_name);

    {
        MUINT32 usensorID = 0;
        if (pSensorGetInfo->SensorId == 0) usensorID = S5K3L6_SENSOR_ID;
        else if (pSensorGetInfo->SensorId == 1) usensorID = S5K4H7YX_SENSOR_ID;
        else if (pSensorGetInfo->SensorId == 2) usensorID = S5K5E9YX_SENSOR_ID;
        else usensorID = 0xFFFF;

        snprintf(mtk_ccm_name, 
		sizeof(mtk_ccm_name),
                    "%s\nSensor ID = %x;", mtk_ccm_name, usensorID);
    }
