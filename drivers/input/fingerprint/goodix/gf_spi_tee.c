...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,fih-fp");
	if (node) {
	...
	gf_dev->pins_ldo_en_high = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fingerprint_ldo_en_high");
	if (IS_ERR(gf_dev->pins_ldo_en_high)) {
		ret = PTR_ERR(gf_dev->pins_ldo_en_high);
		gf_debug(ERR_LOG, "%s can't find fingerprint pins_ldo_en_high\n", __func__);
		return ret;
	}
    gf_dev->pins_ldo_en_low = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fingerprint_ldo_en_low");
	if (IS_ERR(gf_dev->pins_ldo_en_low)) {
		ret = PTR_ERR(gf_dev->pins_ldo_en_low);
		gf_debug(ERR_LOG, "%s can't find fingerprint pins_ldo_en_low\n", __func__);
		return ret;
	}
...
static void gf_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
	...
	/* TODO:  set power  according to actual situation  */
		enable = 0;
#ifdef CONFIG_OF
        pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_ldo_en_high);
...
static int __init gf_init(void)
{
	...
	if(((strstr(saved_command_line, "androidboot.fp=gxfp3258_roo") == NULL)) || (strstr(saved_command_line,"androidboot.mode=charger") != NULL)) {
