...
extern int fih_ili988h_selftest_open(void);
extern int fih_touch_proc_init(void);
extern int fih_ili988h_selftest_read(void);
extern void fih_ili988h_tpfwver_read(char *);
//extern void touch_tpfwimver_read(char *fw_ver);
//extern void touch_fwupgrade(int);
//extern void touch_fwupgrade_read(char *);
extern void fih_ili988h_vendor_read(char *);
extern int fih_ili988h_double_tap_read(void);
extern int fih_ili988h_double_tap_write(int enable);
...
static void tpd_resume(struct device *h)
{
	...
	//hardware have do reset,disable sw reset:for double tap
...
static int ilitek_platform_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    touch_cb.touch_selftest_result = fih_ili988h_selftest_read;
    touch_cb.touch_tpfwver_read = fih_ili988h_tpfwver_read;
...
//    touch_cb.touch_fwupgrade_read = touch_fwupgrade_read;
    touch_cb.touch_vendor_read = fih_ili988h_vendor_read;
    touch_cb.touch_double_tap_read = fih_ili988h_double_tap_read;
    touch_cb.touch_double_tap_write = fih_ili988h_double_tap_write;
...
static int tpd_local_init(void)
{
   ...
   if(((strstr(saved_command_line, "ili9881h_hd720plus_dsi_vdo_dj") == NULL))) {
				   ipio_err("%s this is not ili9881 IC driver\n",__func__);
				   return 0;
}
