...
#define FIH_CSV_PASS_NAME			"fih_mp_pass"
#define FIH_CSV_FAIL_NAME			"fih_mp_fail"
#define FIH_CSV_LCM_PASS_NAME		"fih_mp_lcm_pass"
#define FIH_CSV_LCM_FAIL_NAME		"fih_mp_lcm_fail"
...
int core_mp_ctrl_lcm_status(bool on)
{
	...
	ret = core_i2c_write(core_config->slave_i2c_addr, lcd, ARRAY_SIZE(lcd));
	if (ret < 0) {
...
int core_mp_calc_timing_nodp(void)
{
	...
	ret = core_i2c_write(core_config->slave_i2c_addr, timing_cmd, sizeof(timing_cmd));
	if (ret < 0) {
		ipio_err("Failed to write timing command\n");
		goto out;
	}

	ret = core_i2c_read(core_config->slave_i2c_addr, get_timing, sizeof(get_timing));
	if (ret < 0) {
...
static int allnode_mutual_cdc_data(int index)
{
	...
	res = core_i2c_write(core_config->slave_i2c_addr, cmd, protocol->cdc_len);
	if (res < 0) {
		ipio_err("I2C Write Error while initialising cdc\n");
		goto out;
	}
	...
	if (core_mp->busy_cdc == POLL_CHECK) {
		res = core_config_check_cdc_busy(150);
...
int codeToOhm(int32_t Code)
{
    ...
    else
    {
        ...
        temp = ((douTVCH - douTVCL) * douVariation * (douTDF1 - douTDF2) * (1<<12) / (9 * Code * douCint)) * 100; //fix
...
static int short_test(int index,int frame_index)
{
	...
	ipio_debug(DEBUG_MP_TEST, "short_test codeToOhm %d %d",protocol->major,protocol->mid>=4);
	if(protocol->major >= 5 && protocol->mid >= 4) {
		...
		for (j = 0; j < core_mp->frame_len; j++)
			{
				...
				ipio_debug(DEBUG_MP_TEST, "%d,",frame_buf[j]);
				if(j%18==17)
				{
...
int core_mp_compare_retry_cdc_result(int index)
{
	...
	ipio_info("core_mp_compare_retry_cdc_result index = %d, item = %s\n",index,tItems[index].desp);
	...
	if (combined_flag) {
		ipio_info("Combined data is same, doing retry\n");
		retry_result = MP_FAIL;
	}
...
void core_mp_show_result(void)
{
	...
	if (core_mp->fih_run) {
		if (core_mp->fih_lcm) {
			ret_pass_name = FIH_CSV_LCM_PASS_NAME;
			ret_fail_name = FIH_CSV_LCM_FAIL_NAME;
		} else {
			ret_pass_name = FIH_CSV_PASS_NAME;
			ret_fail_name = FIH_CSV_FAIL_NAME;
		}
	} else {
		...
		if (strncmp(item, tItems[i].desp, strlen(item)) == 0) {
			...
			if (tItems[i].run) {
				...
				if (strnstr(tItems[i].desp, "LCM", strlen(tItems[i].desp)) != NULL) {
					if (!core_mp->fih_run && !core_mp->fih_lcm)
...
#else
	if (core_config_check_cdc_busy(150) < 0) {
...
int core_mp_init(void)
{
	...
	if (!ERR_ALLOC_MEM(core_config->tp_info)) {
		if (core_mp == NULL) {
			...
			core_mp->run = false;
			core_mp->fih_run = false;
			core_mp->fih_lcm = false;
			core_mp->final_result = MP_FAIL;
