	...
	if (pid == protocol->demo_pid) {
		...
		for (i = 0; i < MAX_TOUCH_NUM; i++) {
			...
			if (!core_fr->isSetResolution) {
				...
				g_mutual_data.mtp[g_mutual_data.touch_num].x = TOUCH_SCREEN_X_MAX - nX * TOUCH_SCREEN_X_MAX / TPD_WIDTH;
				g_mutual_data.mtp[g_mutual_data.touch_num].y = TOUCH_SCREEN_Y_MAX - nY * TOUCH_SCREEN_Y_MAX / TPD_HEIGHT;
...
static int finger_report_ver_5_0(void)
{
	...
	{
		ipio_err("waiting for input_register_device tpd!\n");
		goto out;
	}
