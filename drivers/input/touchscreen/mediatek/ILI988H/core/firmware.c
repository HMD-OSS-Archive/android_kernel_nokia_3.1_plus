...
static int verify_flash_data(void)
{
	...
	for (i = 0; i < g_section_len + 1; i++) {
		if ((!core_firmware->isboot &&g_flash_sector[i].data_flag) || (core_firmware->isboot && g_flash_sector[i].inside_block)) {
...
static int convert_hex_array(void)
{
	...
	if (block > 0) {
		...
		for (i = 0; i < block; i++) {
			...
			start_addr = g_flash_block_info[i].start_addr;
			end_addr = g_flash_block_info[i].end_addr;
			g_flash_block_info[i].hex_crc = (CTPM_FW[64 + end_addr - 3] << 24) + (CTPM_FW[64 + end_addr - 2] << 16)+ (CTPM_FW[64 + end_addr - 1] << 8) + CTPM_FW[64 +(end_addr)];
			g_flash_block_info[i].block_crc = tddi_check_data(start_addr, end_addr - start_addr - crc_byte_len + 1);
...
#ifdef BOOT_FW_UPGRADE_READ_HEX
	msleep(2000);  //control upgrade timing
...
#endif
#if 0
	for (i = 0x10000; i < 0x11FFF; i=i+16) {
		ipio_err("flash_fw[0x%x]= 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",i,
			flash_fw[i],flash_fw[i+1],flash_fw[i+2],flash_fw[i+3],flash_fw[i+4],flash_fw[i+5],flash_fw[i+6],flash_fw[i+7],
			flash_fw[i+8],flash_fw[i+9],flash_fw[i+10],flash_fw[i+11],flash_fw[i+12],flash_fw[i+13],flash_fw[i+14],flash_fw[i+15]);
		}
	for (i = 0x12000; i < 0x12FFF; i=i+16) {
		ipio_err("flash_fw[0x%x]= 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",i,
			flash_fw[i],flash_fw[i+1],flash_fw[i+2],flash_fw[i+3],flash_fw[i+4],flash_fw[i+5],flash_fw[i+6],flash_fw[i+7],
			flash_fw[i+8],flash_fw[i+9],flash_fw[i+10],flash_fw[i+11],flash_fw[i+12],flash_fw[i+13],flash_fw[i+14],flash_fw[i+15]);
		}
