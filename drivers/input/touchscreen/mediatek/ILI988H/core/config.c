...
extern int fih_mp_result;
 int double_tap_work_flags=0;

EXPORT_SYMBOL(double_tap_work_flags);
...
int core_config_ice_mode_disable(void)
{
	...
	res = core_i2c_write(core_config->slave_i2c_addr, cmd, 4);
...
int core_config_check_cdc_busy(int delay)
{
	...
	while (timer > 0) {
		...
		ipio_debug(DEBUG_CONFIG, "CDC busy state = 0x%x!Aactual_fw = %d\n", busy,core_fr->actual_fw_mode);
		if (core_fr->actual_fw_mode == P5_0_FIRMWARE_DEMO_MODE && busy == 0x41) {
			ipio_info("Check busy is free\n");
			res = 0;
			break;
		}
		if (core_fr->actual_fw_mode == P5_0_FIRMWARE_TEST_MODE && busy == 0x51) {
			ipio_info("Check busy is free\n");
			res = 0;
			break;
		}
...
int fih_ili988h_selftest_open(void){
	return 0;
}
int fih_ili988h_selftest_read(void){
	//int val=0x00;
	...
	{
		ret=0;
		ipio_debug(DEBUG_FINGER_REPORT, " fih mp test pass %d \n", fih_mp_result);
	}
	else
	{
		ret=1;
		ipio_debug(DEBUG_FINGER_REPORT, " fih mp test fail %d \n", fih_mp_result);
	}
	return ret;
...
void fih_ili988h_tpfwver_read(char *fw_ver)
{
	core_config_get_fw_ver();
	snprintf(fw_ver, PAGE_SIZE,"%d.%d.%d\n", core_config->firmware_ver[1],core_config->firmware_ver[2],core_config->firmware_ver[3]);
}

void fih_ili988h_vendor_read(char *buf)
{
    sprintf(buf, "%s\n","ili988h");
}

int fih_ili988h_double_tap_read(void)
{
	pr_err("read gesture double_tap mode\n");
	return (int)core_config->isEnableGesture;
}


int fih_ili988h_double_tap_write(int enable)
{
	if(enable)
	{
	double_tap_work_flags=1;
	core_config->isEnableGesture = true;
	pr_err("enable gesture double_tap mode\n");
	}
	else
	{
	double_tap_work_flags=0;
	core_config->isEnableGesture = false;
