...
int double_tap_focal_work_flags = 0;
...
int fih_focal8006p_double_tap_read(void)
{
	pr_err("read gesture double_tap mode\n");
	return (int)fts_gesture_data.mode;
}


int fih_focal8006p_double_tap_write(int enable)
{
	if(enable)
	{
	fts_gesture_data.mode = ENABLE;
	double_tap_focal_work_flags = 1;
	pr_err("enable gesture double_tap mode\n");
	}
	else
	{
	fts_gesture_data.mode = DISABLE;
	double_tap_focal_work_flags = 0;
