...
extern int fts_fwupg_get_ver_in_tp(struct i2c_client *client, u8 *ver);

extern struct fih_touch_cb touch_cb;
extern int fih_focal8006p_selftest_open(void);
extern int fih_touch_proc_init(void);
extern int fih_focal8006p_selftest_read(void);
extern void fih_focal8006p_tpfwver_read(char *);
extern void fih_focal8006p_vendor_read(char *);
extern int fih_focal8006p_double_tap_read(void);
extern int fih_focal8006p_double_tap_write(int enable);
int focal_tp_fw = 0;
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	focal_tp_fw = fw_ver_in_tp;
	
    fih_touch_proc_init();
    touch_cb.touch_selftest = fih_focal8006p_selftest_open;
    touch_cb.touch_selftest_result = fih_focal8006p_selftest_read;
    touch_cb.touch_tpfwver_read = fih_focal8006p_tpfwver_read;
    touch_cb.touch_vendor_read = fih_focal8006p_vendor_read;
    touch_cb.touch_double_tap_read = fih_focal8006p_double_tap_read;
    touch_cb.touch_double_tap_write = fih_focal8006p_double_tap_write;
...
static void tpd_suspend(struct device *h)
{
	...
	FTS_ERROR("FTS focal disable VSP VSN!");
...
static int __init tpd_driver_init(void)
{
	...
	if((strstr(saved_command_line, "txd_focal720_dsi_vdo_bflash") != NULL))
	{
    tpd_get_dts_info();
	...
	{
       FTS_INFO("not match lk focal lcm name\n");
		return -1;
	}
}
