...
int double_tap_himax_work_flags=0;
...
uint8_t getFlashCommand(void)
{
 ...
 uint8_t getFlashDumpProgress(void)
{
	return flash_progress;
}

 uint8_t getFlashDumpComplete(void)
{
	return flash_dump_complete;
}

 uint8_t getFlashDumpFail(void)
{
...
void setFlashDumpFail(uint8_t fail)
{
 ...
 void setFlashCommand(uint8_t command)
{
	flash_command = command;
}

 void setFlashReadStep(uint8_t step)
{
...
int fih_himax_selftest_read(void){
	int val=0x00;
...
void fih_himax_tpfwver_read(char *fw_ver)
{

//snprintf(fw_ver, 64, "truly:himax83102D:0x%2.2X \n", ic_data->vendor_touch_cfg_ver);
#if 0
			...
			himax_int_enable(1);
#endif
			printk("ic_data->vendor_touch_cfg_ver:0x%2.2x\r\n",ic_data->vendor_touch_cfg_ver);
			snprintf(fw_ver, 64, "0x%2.2X \n", ic_data->vendor_touch_cfg_ver);

/*
			if(strstr(saved_command_line, "hx83102_hd720plus_dsi_vdo_dzero_flash_hlt") != NULL){
				//printk("ic_data->vendor_touch_cfg_ver:0x2.2x\r\n",ic_data->vendor_touch_cfg_ver);
				snprintf(fw_ver, 64, "hlt:himax83102D:0x%2.2X \n", ic_data->vendor_touch_cfg_ver);

			}
			else if((strstr(saved_command_line, "hx83102_hd720plus_dsi_vdo_dzero_flash_truly") != NULL)){
				snprintf(fw_ver, 64, "truly:himax83102D:0x%2.2X \n", ic_data->vendor_touch_cfg_ver);
			}
	*/
...
void fih_himax_vendor_read(char *buf)
{
	...
	if(strstr(saved_command_line, "hx83102_hd720plus_dsi_vdo_dzero_flash_hlt") != NULL){
					...
					sprintf(buf, "%s\n","truly:himax83102Dzero");
	}


    
	else if(strstr(saved_command_line, "hx83102_hd720plus_dsi_vdo_bflash_hlt") != NULL){
						
						sprintf(buf, "%s\n","hlt:himax83102bflash");
		
   }

   else if(strstr(saved_command_line, "txd_hd720plus_dsi_vdo_bflash") != NULL){
						
						sprintf(buf, "%s\n","txd:himax83102bflash");
		
   }	
    else if((strstr(saved_command_line, "hx83102_hd720plus_dsi_vdo_bflash_truly") != NULL)){
						sprintf(buf, "%s\n","truly:himax83102bflash");
		}
...
int fih_himax_double_tap_read(void)
{
	struct himax_ts_data *ts = private_ts;

	pr_err("read gesture double_tap mode\n");
	return ts->SMWP_enable;
   
}

int fih_himax_double_tap_write(int enable)
{
	...
	if (!enable) {
		double_tap_himax_work_flags=0;
		ts->SMWP_enable = 0;
		for (i = 0; i < 16; i++) {
			ts->gesture_cust_en[i] = 0;
		}
		pr_err("disable gesture double_tap mode\n");
	} else if (enable) {
	   double_tap_himax_work_flags=1;
		ts->SMWP_enable = 1;
		for (i = 0; i < 16; i++) {
			ts->gesture_cust_en[i] = 1;
		}
		pr_err("enable gesture double_tap mode\n");
	}	else
		return -EINVAL;
