...
struct himax_on_core_command_operation *g_on_core_cmd_op = NULL;
struct on_ic_operation *on_pic_op = NULL;
struct on_fw_operation *on_pfw_op = NULL;
struct on_flash_operation *on_pflash_op = NULL;
struct on_sram_operation *on_psram_op = NULL;
struct on_driver_operation *on_pdriver_op = NULL;
...
static void himax_mcu_burst_enable(uint8_t auto_add_4_byte)
{
	...
	if (himax_bus_write(on_pic_op->addr_conti[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}
	...
	if (himax_bus_write(on_pic_op->addr_incr4[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}
}
...
static int himax_mcu_register_read(uint8_t *read_addr, uint32_t read_length, uint8_t *read_data, uint8_t cfg_flag)
{
	...
	if (cfg_flag == 0 || cfg_flag == 2) {
		...
		if (cfg_flag == 2)
			g_core_fp.fp_idle_mode(1); /*disable idle mode to read register*/
		...
		if (himax_bus_write(on_pic_op->addr_ahb_addr_byte_0[0], tmp_data, FOUR_BYTE_DATA_SZ, HIMAX_I2C_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
			goto i2c_rw_fail;
		}

		tmp_data[0] = on_pic_op->data_ahb_access_direction_read[0];
		if (himax_bus_write(on_pic_op->addr_ahb_access_direction[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
			goto i2c_rw_fail;
		}

		if (himax_bus_read(on_pic_op->addr_ahb_rdata_byte_0[0], read_data, read_length, HIMAX_I2C_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
			goto i2c_rw_fail;
		}
...
i2c_rw_fail:
		if (cfg_flag == 2)
			g_core_fp.fp_idle_mode(0); /*enble idle mode after read register*/
...
static int himax_mcu_flash_write_burst(uint8_t *reg_byte, uint8_t *write_data)
{
	...
	if (himax_bus_write(on_pic_op->addr_ahb_addr_byte_0[0], data_byte, data_byte_sz, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return I2C_FAIL;
	}
	return NO_ERR;
...
static void himax_mcu_flash_write_burst_lenth(uint8_t *reg_byte, uint8_t *write_data, uint32_t length)
{
	...
	if (himax_bus_write(on_pic_op->addr_ahb_addr_byte_0[0], data_byte, length + FOUR_BYTE_ADDR_SZ, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		kfree(data_byte);
		return;
	}
...
static void himax_mcu_interface_on(void)
{
	...
	if (himax_bus_read(on_pic_op->addr_ahb_rdata_byte_0[0], tmp_data, FOUR_BYTE_DATA_SZ, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}
	...
	do {
		...
		/*Check cmd*/
		himax_bus_read(on_pic_op->addr_conti[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
		himax_bus_read(on_pic_op->addr_incr4[0], tmp_data2, 1, HIMAX_I2C_RETRY_TIMES);

		if (tmp_data[0] == on_pic_op->data_conti[0] && tmp_data2[0] == on_pic_op->data_incr4[0])
			break;
...
static bool himax_mcu_wait_wip(int Timing)
{
	...
	do {
		g_core_fp.fp_register_read(on_pflash_op->addr_ctrl_auto, FOUR_BYTE_DATA_SZ, tmp_data, 0);
		...
		if (tmp_data[1] == on_pflash_op->data_auto[0] && (tmp_data[0] == on_pflash_op->data_main_read[0] ||
			tmp_data[0] == on_pflash_op->data_sfr_read[0] || tmp_data[0] == on_pflash_op->data_spp_read[0])) {
...
static void himax_mcu_sense_on(uint8_t FlashMode)
{
	...
	if (himax_bus_write(on_pic_op->adr_mcu_ctrl[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
	}
}
...
{
	g_core_fp.fp_register_write(on_pic_op->adr_tcon_ctrl, FOUR_BYTE_DATA_SZ, on_pic_op->cmd_tcon_on, 0);
	return;
}
...
static bool himax_mcu_watch_dog_off(void)
{
	...
	do {
		g_core_fp.fp_register_write(on_pic_op->adr_wdg_ctrl, FOUR_BYTE_DATA_SZ, on_pic_op->cmd_wdg_psw, 0);
		g_core_fp.fp_register_read(on_pic_op->adr_wdg_ctrl, FOUR_BYTE_DATA_SZ, tmp_data, 0);

		I("%s:Read status from IC = %X,%X\n", __func__, tmp_data[0], tmp_data[1]);
	} while ((tmp_data[1] != on_pic_op->cmd_wdg_psw[1] || tmp_data[0] != on_pic_op->cmd_wdg_psw[0]) && retry++ < 5);
	...
	if (retry >= 5) {
		...
		g_core_fp.fp_register_write(on_pic_op->adr_wdg_cnt_ctrl, FOUR_BYTE_DATA_SZ, on_pic_op->cmd_wdg_cnt_clr, 0);
		ret = true;
	}
	return ret;
}
...
static bool himax_mcu_sense_off(void)
{
	...
	if (g_core_fp.fp_watch_dog_off()) {
		if (himax_bus_write(on_pic_op->adr_mcu_ctrl[0], on_pic_op->cmd_mcu_off, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
			ret = false;
		} else {
...
{
	/*tmp_data[0] = 0x80;*/
	if (himax_bus_write(on_pic_op->adr_sleep_ctrl[0], on_pic_op->cmd_sleep_in, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
	}
}
...
static void diag_mcu_parse_raw_data(struct himax_report_data *hx_touch_data, int mul_num, int self_num, uint8_t diag_cmd, int32_t *mutual_data, int32_t *self_data)
{
	...
	if (hx_touch_data->hx_rawdata_buf[0] == on_pfw_op->data_rawdata_ready_lb[0]
	    && hx_touch_data->hx_rawdata_buf[1] == on_pfw_op->data_rawdata_ready_hb[0]
...
static uint32_t himax_mcu_check_CRC(uint8_t *start_addr, int reload_length)
{
	...
	g_core_fp.fp_flash_write_burst(on_pfw_op->addr_flash_checksum, on_pfw_op->data_flash_checksum);
	if (ret < NO_ERR) {
		E("%s: i2c access fail!\n", __func__);
		return HW_CRC_FAIL;
	}
	...
	do {
		g_core_fp.fp_register_read(on_pfw_op->addr_flash_checksum, FOUR_BYTE_DATA_SZ, tmp_data, 0);
		if (ret < NO_ERR) {
			E("%s: i2c access fail!\n", __func__);
			return HW_CRC_FAIL;
		}
	...
	g_core_fp.fp_register_read(on_pfw_op->addr_crc_value, FOUR_BYTE_DATA_SZ, tmp_data, 0); /*50*/
	if (ret < NO_ERR) {
		E("%s: i2c access fail!\n", __func__);
		return HW_CRC_FAIL;
	}
...
static void himax_mcu_set_SMWP_enable(uint8_t SMWP_enable, bool suspended)
{
	...
	do {
		if (SMWP_enable == 0x01 && suspended) {
			...
			himax_bus_write(on_pfw_op->addr_smwp_enable[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
		...
		himax_bus_read(on_pfw_op->addr_smwp_enable[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES);

		retry_cnt++;
	} while ((tmp_data[0] != back_data[0]) && retry_cnt < HIMAX_REG_RETRY_TIMES);
}
...
static void himax_mcu_usb_detect_set(uint8_t *cable_config)
{
	...
	do {
		if (cable_config[1] == 0x01) {
			...
			himax_bus_write(on_pfw_op->addr_usb_detect[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
		...
		himax_bus_read(on_pfw_op->addr_usb_detect[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
...
static void himax_mcu_diag_register_set(uint8_t diag_command, uint8_t storage_type)
{
	...
	if (himax_bus_write(on_pfw_op->addr_raw_out_sel[0], tmp_data, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}
}
...
static void himax_mcu_control_reK(bool enable)
{
	...
	g_core_fp.fp_register_read(on_pfw_op->addr_reK_crtl, FOUR_BYTE_DATA_SZ, tmp_data, 0);
	...
	g_core_fp.fp_register_write(on_pfw_op->addr_reK_crtl, FOUR_BYTE_DATA_SZ, tmp_data, 0);
}
...
static int himax_dc_compare(u8 *thx_data, u16 *info_data16)
{
	...
	uint16_t mul_dc_ava = (uint16_t)thx_data[0] | ((uint16_t)thx_data[1] << 8);
	uint16_t slf_dc_ava = (uint16_t)thx_data[3] | ((uint16_t)thx_data[4] << 8);
	...
	for (i = 0; i < x_num * y_num; i++) {
		if (info_data16[i] > mul_dc_upper || info_data16[i] < mul_dc_lower) {
			++mul_fail_cnt;
			result |= MASK_BIT_0;
		}
	}
	...
	for (i = x_num * y_num; i < x_num * y_num + x_num + y_num; i++) {
		if (info_data16[i] > slf_dc_upper || info_data16[i] < slf_dc_lower) {
			++slf_fail_cnt;
			result |= MASK_BIT_1;
		}
	}
...
static int himax_bank_compare(u8 *thx_data, u16 *info_data16)
{
	...
	for (i = x_num * y_num; i < x_num * y_num + x_num + y_num; i++) {
		if (info_data16[i] > slf_up_bound || info_data16[i] < slf_low_bound) {
			++slf_fail_cnt;
			result |= MASK_BIT_2;
		}
	}
...
{
	uint8_t data[EIGHT_BYTE_DATA_SZ] = {0};
	int i = 0;
	int flag = 0;
	uint8_t result = 0x00;
#ifdef HX_TP_SELF_TEST_DRIVER
	...
	himax_bus_read(on_pfw_op->addr_criteria_addr[0], data, EIGHT_BYTE_DATA_SZ, HIMAX_I2C_RETRY_TIMES);/*read test criteria*/
	for (i = 0; i < EIGHT_BYTE_DATA_SZ; i++) {
		if (data[i] == 0x00)
			flag++;
	}
	if (flag == EIGHT_BYTE_DATA_SZ) {
		data[0] = on_pfw_op->data_thx_avg_mul_dc_lsb[0];
		data[1] = on_pfw_op->data_thx_avg_mul_dc_msb[0];
		data[2] = on_pfw_op->data_thx_mul_dc_up_low_bud[0]; /*2850 = 0xB22 , 100%*/
		data[3] = on_pfw_op->data_thx_avg_slf_dc_lsb[0];
		data[4] = on_pfw_op->data_thx_avg_slf_dc_msb[0];
		data[5] = on_pfw_op->data_thx_slf_dc_up_low_bud[0]; /*1300 = 0x514 , 100%*/
	...
	info_data16 = kzalloc(sizeof(uint16_t) * total_size / 2, GFP_KERNEL);
	...
	g_core_fp.fp_diag_register_set(on_pfw_op->data_bank_set[0], 0x01);
	g_core_fp.fp_get_DSRAM_data(info_data, 0);
	himax_8bit_to_16bit(info_data, total_size, info_data16);
	...
	himax_bus_write(on_pfw_op->addr_selftest_addr_en[0], data, 1, HIMAX_I2C_RETRY_TIMES);

	do {
		himax_bus_read(on_pfw_op->addr_selftest_result_addr[0], data, EIGHT_BYTE_DATA_SZ, HIMAX_I2C_RETRY_TIMES);
		msleep(20);
		cnt++;
...
{
	if (!disable) {
		if (himax_bus_write(on_pfw_op->addr_fw_mode_status[0], on_pfw_op->data_idle_en_pwd, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}
	} else {
		if (himax_bus_write(on_pfw_op->addr_fw_mode_status[0], on_pfw_op->data_idle_dis_pwd, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}
	}
...
static int himax_mcu_read_ic_trigger_type(void)
{
	...
	g_core_fp.fp_register_read(on_pfw_op->addr_trigger_addr, FOUR_BYTE_DATA_SZ, tmp_data, 0);
...
{
	uint8_t data[EIGHT_BYTE_DATA_SZ];
	int i = 0;

	for (i = 0; i < 6 ; i++) {
		if (himax_bus_read(on_pfw_op->addr_fw_ver_start[0] + i, &data[i], 1, HIMAX_I2C_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
...
static bool himax_mcu_read_event_stack(uint8_t *buf, uint8_t length)
{
	...
	if (himax_bus_write(on_pfw_op->addr_ahb_addr[0], cmd, 1, HIMAX_I2C_RETRY_TIMES) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return 0;
	}

	himax_bus_read(on_pfw_op->addr_event_addr[0], buf, length, HIMAX_I2C_RETRY_TIMES);
...
static void himax_mcu_return_event_stack(void)
{
	...
	do {
		...
		g_core_fp.fp_flash_write_burst(on_psram_op->addr_rawdata_addr, tmp_data);
		g_core_fp.fp_register_read(on_psram_op->addr_rawdata_addr, FOUR_BYTE_DATA_SZ, tmp_data, 0);
		retry--;
		msleep(10);
	} while ((tmp_data[1] != on_psram_op->addr_rawdata_end[1] && tmp_data[0] != on_psram_op->addr_rawdata_end[0]) && retry > 0);
...
{
	g_core_fp.fp_register_write(on_pflash_op->addr_ctrl_auto, FOUR_BYTE_DATA_SZ, on_pflash_op->data_main_erase, 0);
}
...
static bool himax_mcu_ahb_squit(void)
{
	...
	g_core_fp.fp_register_write(on_pflash_op->addr_ahb_ctrl, FOUR_BYTE_DATA_SZ, on_pflash_op->data_ahb_squit, 0);
	msleep(1);
	...
	do {
		...
		msleep(1);
		g_core_fp.fp_register_read(on_pflash_op->addr_ahb_ctrl, FOUR_BYTE_DATA_SZ, tmp_data, 0);
	} while (tmp_data[0] != 0x00);

	return true;
}

bool himax_mcu_sfr_rw(uint8_t *addr, int length, uint8_t *data, uint8_t rw_ctrl) /*r:0, w:1*/
{
	uint8_t tmp_data[FOUR_BYTE_DATA_SZ];
	int i = 0;
	...
	g_core_fp.fp_register_write(on_pflash_op->addr_ctrl_auto, FOUR_BYTE_DATA_SZ, tmp_data, 0);
	msleep(1);
	...
	if (!rw_ctrl) {
		...
		I("%s: Rrite SFR! data[0] = 0x%02X, data[1] = 0x%02X, data[2] = 0x%02X, data[3] = 0x%02X\n"
		, __func__, data[0], data[1], data[2], data[3]);
	} else {
		g_core_fp.fp_register_write(addr, length, data, 0);
		msleep(1);
		if (g_core_fp.fp_wait_wip(5)) {
			g_core_fp.fp_sfr_rw(addr, FOUR_BYTE_DATA_SZ, tmp_data, 0);
			for (i = 0; i < FOUR_BYTE_DATA_SZ; i++) {
				if (tmp_data[i] != data[i]) {
					g_core_fp.fp_ahb_squit();
					E("%s: Write SFR data fail! data[%d] = 0x%02X, read_data[%d] = 0x%02X\n", __func__, i, data[i], i, tmp_data[i]);
					return false;
				}
			}
			if (!g_core_fp.fp_ahb_squit())
...
bool himax_mcu_unlock_flash(void)
{
	...
	if (!g_core_fp.fp_sfr_rw(on_pflash_op->addr_unlock_0, FOUR_BYTE_DATA_SZ, on_pflash_op->data_cmd0, 1))
		return false;

	if (!g_core_fp.fp_sfr_rw(on_pflash_op->addr_unlock_4, FOUR_BYTE_DATA_SZ, on_pflash_op->data_cmd1, 1))
		return false;

	if (!g_core_fp.fp_sfr_rw(on_pflash_op->addr_unlock_8, FOUR_BYTE_DATA_SZ, on_pflash_op->data_cmd2, 1))
		return false;

	if (!g_core_fp.fp_sfr_rw(on_pflash_op->addr_unlock_c, FOUR_BYTE_DATA_SZ, on_pflash_op->data_cmd3, 1))
		return false;
...
bool himax_mcu_lock_flash(void)
{
	...
	if (!g_core_fp.fp_sfr_rw(on_pflash_op->addr_unlock_c, FOUR_BYTE_DATA_SZ, tmp_data, 0))
		return false;
	...
	if (!g_core_fp.fp_sfr_rw(on_pflash_op->addr_unlock_c, FOUR_BYTE_DATA_SZ, tmp_data, 1))
		return false;
...
static void himax_mcu_flash_programming(uint8_t *FW_content, int FW_Size)
{
	...
	for (s_idx = start_sector_idx; s_idx < end_sector_idx; s_idx++) {
		...
		for (p_idx = p_tmp_idx ; p_idx < p_end_idx ; p_idx++) {
			...
			memcpy(tmp_data, &ImageBuffer[prog_tmp_idx], page_size * sizeof(uint8_t));
			/*I("%s: Sector idx = %d, Page idx = %d, prog_tmp_idx = %X\n", __func__, s_idx, p_idx, prog_tmp_idx);*/
			tmp_addr[3] = 0x00;
			tmp_addr[2] = 0x00;
			tmp_addr[1] = (s_idx << 3) + (p_idx >> 3);
			tmp_addr[0] = (p_idx % 0x08) << 5;
			g_core_fp.fp_flash_page_write(tmp_addr, page_size, tmp_data);
		}
	}
}
...
static void himax_mcu_flash_page_write(uint8_t *write_addr, int length, uint8_t *write_data)
{
	...
	g_core_fp.fp_register_write(on_pflash_op->addr_ctrl_base, FOUR_BYTE_DATA_SZ, write_addr, 0);
...
static int himax_mcu_fts_ctpm_fw_upgrade_with_sys_fs_64k(unsigned char *fw, int len, bool change_iref)
{
	...
	CRC_result = g_core_fp.fp_check_CRC(on_pfw_op->addr_program_reload_from, len);
...
static void himax_mcu_flash_read(uint8_t *r_data, int start_addr, int length)
{
	uint8_t tmp_addr[FOUR_BYTE_DATA_SZ];
	uint8_t tmp_data[FLASH_RW_MAX_LEN] = {0};
	...
	for (addr = start_addr; addr < start_addr + length; addr = addr + FW_PAGE_SEZE) {
		I("%s: addr = %X, i = %d \n", __func__, addr, i);
...
static void himax_mcu_get_DSRAM_data(uint8_t *info_data, bool DSRAM_Flag)
{
	...
	fw_run_flag = himax_write_read_reg(on_psram_op->addr_rawdata_addr, tmp_data, on_psram_op->passwrd_end[1], on_psram_op->passwrd_end[0]);
	...
	for (i = 0; i < total_read_times; i++) {
		...
		on_psram_op->addr_rawdata_addr[0] + i * max_i2c_size;
	...
	if (DSRAM_Flag == false) {
		...
		g_core_fp.fp_flash_write_burst(on_psram_op->addr_rawdata_addr, on_psram_op->data_fin);
	} else {
		/*I("Continue to SRAM!\n");*/
		g_core_fp.fp_flash_write_burst(on_psram_op->addr_rawdata_addr, on_psram_op->data_conti);
	}
	...
	if (check_sum_cal % 0x10000 != 0) {
		...
		memcpy(&info_data[mutual_data_size], &temp_info_data[mutual_data_size + 4], self_data_size * sizeof(uint8_t));
		/*I("%s checksum PASS \n", __func__);*/
	}
...
static void himax_mcu_touch_information(void)
{
	...
	g_core_fp.fp_register_read(on_pdriver_op->addr_fw_rx_tx_maxpt_num, FOUR_BYTE_DATA_SZ, data, 0);
	...
	ic_data->HX_RX_NUM,ic_data->HX_TX_NUM, ic_data->HX_BT_NUM,ic_data->HX_MAX_PT);*/

	/*cmd[3] = 0x08; cmd[2] = 0x00; cmd[1] = 0x00; cmd[0] = 0x0C;*/
	g_core_fp.fp_register_read(on_pdriver_op->addr_fw_xy_rev_int_edge, FOUR_BYTE_DATA_SZ, data, 0);
	if ((data[2] & 0x10) == 0x10) {
	/*0x0800000E, bit[4]*/
	...
	g_core_fp.fp_register_read(on_pdriver_op->addr_fw_define_x_y_res, FOUR_BYTE_DATA_SZ, data, 0);
...
#endif
	I("%s:HX_RX_NUM =%d, HX_TX_NUM =%d, HX_BT_NUM=%d, HX_MAX_PT=%d \n",
	__func__, ic_data->HX_RX_NUM, ic_data->HX_TX_NUM, ic_data->HX_BT_NUM, ic_data->HX_MAX_PT);
...
{
	I("%s: Entering!\n", __func__);
	g_on_core_cmd_op = kzalloc(sizeof(struct himax_on_core_command_operation), GFP_KERNEL);
	g_on_core_cmd_op->ic_op = kzalloc(sizeof(struct on_ic_operation), GFP_KERNEL);
	g_on_core_cmd_op->fw_op = kzalloc(sizeof(struct on_fw_operation), GFP_KERNEL);
	g_on_core_cmd_op->flash_op = kzalloc(sizeof(struct on_flash_operation), GFP_KERNEL);
	g_on_core_cmd_op->sram_op = kzalloc(sizeof(struct on_sram_operation), GFP_KERNEL);
	g_on_core_cmd_op->driver_op = kzalloc(sizeof(struct on_driver_operation), GFP_KERNEL);
...
void himax_mcu_on_cmd_init(void)
{
	I("%s: Entering!\n", __func__);
#ifdef CORE_IC
	himax_on_parse_assign_cmd(on_ic_adr_ahb_addr_byte_0, on_pic_op->addr_ahb_addr_byte_0, sizeof(on_pic_op->addr_ahb_addr_byte_0));
	himax_on_parse_assign_cmd(on_ic_adr_ahb_rdata_byte_0, on_pic_op->addr_ahb_rdata_byte_0, sizeof(on_pic_op->addr_ahb_rdata_byte_0));
	himax_on_parse_assign_cmd(on_ic_adr_ahb_access_direction, on_pic_op->addr_ahb_access_direction, sizeof(on_pic_op->addr_ahb_access_direction));
	himax_on_parse_assign_cmd(on_ic_adr_conti, on_pic_op->addr_conti, sizeof(on_pic_op->addr_conti));
	himax_on_parse_assign_cmd(on_ic_adr_incr4, on_pic_op->addr_incr4, sizeof(on_pic_op->addr_incr4));
	himax_on_parse_assign_cmd(on_ic_cmd_ahb_access_direction_read, on_pic_op->data_ahb_access_direction_read, sizeof(on_pic_op->data_ahb_access_direction_read));
	himax_on_parse_assign_cmd(on_ic_cmd_conti, on_pic_op->data_conti, sizeof(on_pic_op->data_conti));
	himax_on_parse_assign_cmd(on_ic_cmd_incr4, on_pic_op->data_incr4, sizeof(on_pic_op->data_incr4));
	himax_on_parse_assign_cmd(on_ic_adr_mcu_ctrl, on_pic_op->adr_mcu_ctrl, sizeof(on_pic_op->adr_mcu_ctrl));
	himax_on_parse_assign_cmd(on_ic_cmd_mcu_on, on_pic_op->cmd_mcu_on, sizeof(on_pic_op->cmd_mcu_on));
	himax_on_parse_assign_cmd(on_ic_cmd_mcu_off, on_pic_op->cmd_mcu_off, sizeof(on_pic_op->cmd_mcu_off));
	himax_on_parse_assign_cmd(on_ic_adr_sleep_ctrl, on_pic_op->adr_sleep_ctrl, sizeof(on_pic_op->adr_sleep_ctrl));
	himax_on_parse_assign_cmd(on_ic_cmd_sleep_in, on_pic_op->cmd_sleep_in, sizeof(on_pic_op->cmd_sleep_in));
	himax_on_parse_assign_cmd(on_ic_adr_tcon_ctrl, on_pic_op->adr_tcon_ctrl, sizeof(on_pic_op->adr_tcon_ctrl));
	himax_on_parse_assign_cmd(on_ic_cmd_tcon_on, on_pic_op->cmd_tcon_on, sizeof(on_pic_op->cmd_tcon_on));
	himax_on_parse_assign_cmd(on_ic_adr_wdg_ctrl, on_pic_op->adr_wdg_ctrl, sizeof(on_pic_op->adr_wdg_ctrl));
	himax_on_parse_assign_cmd(on_ic_cmd_wdg_psw, on_pic_op->cmd_wdg_psw, sizeof(on_pic_op->cmd_wdg_psw));
	himax_on_parse_assign_cmd(on_ic_adr_wdg_cnt_ctrl, on_pic_op->adr_wdg_cnt_ctrl, sizeof(on_pic_op->adr_wdg_cnt_ctrl));
	himax_on_parse_assign_cmd(on_ic_cmd_wdg_cnt_clr, on_pic_op->cmd_wdg_cnt_clr, sizeof(on_pic_op->cmd_wdg_cnt_clr));
#endif
#ifdef CORE_FW
	himax_on_parse_assign_cmd(on_fw_addr_smwp_enable, on_pfw_op->addr_smwp_enable, sizeof(on_pfw_op->addr_smwp_enable));
	himax_on_parse_assign_cmd(on_fw_addr_program_reload_from, on_pfw_op->addr_program_reload_from, sizeof(on_pfw_op->addr_program_reload_from));
	himax_on_parse_assign_cmd(on_fw_addr_raw_out_sel, on_pfw_op->addr_raw_out_sel, sizeof(on_pfw_op->addr_raw_out_sel));
	himax_on_parse_assign_cmd(on_fw_addr_flash_checksum, on_pfw_op->addr_flash_checksum, sizeof(on_pfw_op->addr_flash_checksum));
	himax_on_parse_assign_cmd(on_fw_data_flash_checksum, on_pfw_op->data_flash_checksum, sizeof(on_pfw_op->data_flash_checksum));
	himax_on_parse_assign_cmd(on_fw_addr_crc_value, on_pfw_op->addr_crc_value, sizeof(on_pfw_op->addr_crc_value));
	himax_on_parse_assign_cmd(on_fw_addr_fw_mode_status, on_pfw_op->addr_fw_mode_status, sizeof(on_pfw_op->addr_fw_mode_status));
	himax_on_parse_assign_cmd(on_fw_addr_icid_addr, on_pfw_op->addr_icid_addr, sizeof(on_pfw_op->addr_icid_addr));
	himax_on_parse_assign_cmd(on_fw_addr_trigger_addr, on_pfw_op->addr_trigger_addr, sizeof(on_pfw_op->addr_trigger_addr));
	himax_on_parse_assign_cmd(on_fw_addr_fw_ver_start, on_pfw_op->addr_fw_ver_start, sizeof(on_pfw_op->addr_fw_ver_start));
	himax_on_parse_assign_cmd(on_fw_data_safe_mode_release_pw_active, on_pfw_op->data_safe_mode_release_pw_active, sizeof(on_pfw_op->data_safe_mode_release_pw_active));
	himax_on_parse_assign_cmd(on_fw_addr_criteria_addr, on_pfw_op->addr_criteria_addr, sizeof(on_pfw_op->addr_criteria_addr));
	himax_on_parse_assign_cmd(on_fw_data_selftest_pass, on_pfw_op->data_selftest_pass, sizeof(on_pfw_op->data_selftest_pass));
	himax_on_parse_assign_cmd(on_fw_addr_reK_crtl, on_pfw_op->addr_reK_crtl, sizeof(on_pfw_op->addr_reK_crtl));
	himax_on_parse_assign_cmd(on_fw_data_reK_en, on_pfw_op->data_reK_en, sizeof(on_pfw_op->data_reK_en));
	himax_on_parse_assign_cmd(on_fw_data_reK_dis, on_pfw_op->data_reK_dis, sizeof(on_pfw_op->data_reK_dis));
	himax_on_parse_assign_cmd(on_fw_data_rst_init, on_pfw_op->data_rst_init, sizeof(on_pfw_op->data_rst_init));
	himax_on_parse_assign_cmd(on_fw_data_dc_set, on_pfw_op->data_dc_set, sizeof(on_pfw_op->data_dc_set));
	himax_on_parse_assign_cmd(on_fw_data_bank_set, on_pfw_op->data_bank_set, sizeof(on_pfw_op->data_bank_set));
	himax_on_parse_assign_cmd(on_fw_addr_selftest_addr_en, on_pfw_op->addr_selftest_addr_en, sizeof(on_pfw_op->addr_selftest_addr_en));
	himax_on_parse_assign_cmd(on_fw_addr_selftest_result_addr, on_pfw_op->addr_selftest_result_addr, sizeof(on_pfw_op->addr_selftest_result_addr));
	himax_on_parse_assign_cmd(on_fw_data_selftest_request, on_pfw_op->data_selftest_request, sizeof(on_pfw_op->data_selftest_request));
	himax_on_parse_assign_cmd(on_fw_data_thx_avg_mul_dc_lsb, on_pfw_op->data_thx_avg_mul_dc_lsb, sizeof(on_pfw_op->data_thx_avg_mul_dc_lsb));
	himax_on_parse_assign_cmd(on_fw_data_thx_avg_mul_dc_msb, on_pfw_op->data_thx_avg_mul_dc_msb, sizeof(on_pfw_op->data_thx_avg_mul_dc_msb));
	himax_on_parse_assign_cmd(on_fw_data_thx_mul_dc_up_low_bud, on_pfw_op->data_thx_mul_dc_up_low_bud, sizeof(on_pfw_op->data_thx_mul_dc_up_low_bud));
	himax_on_parse_assign_cmd(on_fw_data_thx_avg_slf_dc_lsb, on_pfw_op->data_thx_avg_slf_dc_lsb, sizeof(on_pfw_op->data_thx_avg_slf_dc_lsb));
	himax_on_parse_assign_cmd(on_fw_data_thx_avg_slf_dc_msb, on_pfw_op->data_thx_avg_slf_dc_msb, sizeof(on_pfw_op->data_thx_avg_slf_dc_msb));
	himax_on_parse_assign_cmd(on_fw_data_thx_slf_dc_up_low_bud, on_pfw_op->data_thx_slf_dc_up_low_bud, sizeof(on_pfw_op->data_thx_slf_dc_up_low_bud));
	himax_on_parse_assign_cmd(on_fw_data_thx_slf_bank_up, on_pfw_op->data_thx_slf_bank_up, sizeof(on_pfw_op->data_thx_slf_bank_up));
	himax_on_parse_assign_cmd(on_fw_data_thx_slf_bank_low, on_pfw_op->data_thx_slf_bank_low, sizeof(on_pfw_op->data_thx_slf_bank_low));
	himax_on_parse_assign_cmd(on_fw_data_idle_dis_pwd, on_pfw_op->data_idle_dis_pwd, sizeof(on_pfw_op->data_idle_dis_pwd));
	himax_on_parse_assign_cmd(on_fw_data_idle_en_pwd, on_pfw_op->data_idle_en_pwd, sizeof(on_pfw_op->data_idle_en_pwd));
	himax_on_parse_assign_cmd(on_fw_data_rawdata_ready_hb, on_pfw_op->data_rawdata_ready_hb, sizeof(on_pfw_op->data_rawdata_ready_hb));
	himax_on_parse_assign_cmd(on_fw_data_rawdata_ready_lb, on_pfw_op->data_rawdata_ready_lb, sizeof(on_pfw_op->data_rawdata_ready_lb));
	himax_on_parse_assign_cmd(on_fw_addr_ahb_addr, on_pfw_op->addr_ahb_addr, sizeof(on_pfw_op->addr_ahb_addr));
	himax_on_parse_assign_cmd(on_fw_data_ahb_dis, on_pfw_op->data_ahb_dis, sizeof(on_pfw_op->data_ahb_dis));
	himax_on_parse_assign_cmd(on_fw_data_ahb_en, on_pfw_op->data_ahb_en, sizeof(on_pfw_op->data_ahb_en));
	himax_on_parse_assign_cmd(on_fw_addr_event_addr, on_pfw_op->addr_event_addr, sizeof(on_pfw_op->addr_event_addr));
	himax_on_parse_assign_cmd(on_fw_usb_detect_addr, on_pfw_op->addr_usb_detect, sizeof(on_pfw_op->addr_usb_detect));
#endif
#ifdef CORE_FLASH
	himax_on_parse_assign_cmd(on_flash_addr_ctrl_base, on_pflash_op->addr_ctrl_base, sizeof(on_pflash_op->addr_ctrl_base));
	himax_on_parse_assign_cmd(on_flash_addr_ctrl_auto, on_pflash_op->addr_ctrl_auto, sizeof(on_pflash_op->addr_ctrl_auto));
	himax_on_parse_assign_cmd(on_flash_data_main_erase, on_pflash_op->data_main_erase, sizeof(on_pflash_op->data_main_erase));
	himax_on_parse_assign_cmd(on_flash_data_auto, on_pflash_op->data_auto, sizeof(on_pflash_op->data_auto));
	himax_on_parse_assign_cmd(on_flash_data_main_read, on_pflash_op->data_main_read, sizeof(on_pflash_op->data_main_read));
	himax_on_parse_assign_cmd(on_flash_data_page_write, on_pflash_op->data_page_write, sizeof(on_pflash_op->data_page_write));
	himax_on_parse_assign_cmd(on_flash_data_sfr_read, on_pflash_op->data_sfr_read, sizeof(on_pflash_op->data_sfr_read));
	himax_on_parse_assign_cmd(on_flash_data_spp_read, on_pflash_op->data_spp_read, sizeof(on_pflash_op->data_spp_read));
	himax_on_parse_assign_cmd(on_flash_addr_ahb_ctrl, on_pflash_op->addr_ahb_ctrl, sizeof(on_pflash_op->addr_ahb_ctrl));
	himax_on_parse_assign_cmd(on_flash_data_ahb_squit, on_pflash_op->data_ahb_squit, sizeof(on_pflash_op->data_ahb_squit));
	himax_on_parse_assign_cmd(on_flash_addr_unlock_0, on_pflash_op->addr_unlock_0, sizeof(on_pflash_op->addr_unlock_0));
	himax_on_parse_assign_cmd(on_flash_addr_unlock_4, on_pflash_op->addr_unlock_4, sizeof(on_pflash_op->addr_unlock_4));
	himax_on_parse_assign_cmd(on_flash_addr_unlock_8, on_pflash_op->addr_unlock_8, sizeof(on_pflash_op->addr_unlock_8));
	himax_on_parse_assign_cmd(on_flash_addr_unlock_c, on_pflash_op->addr_unlock_c, sizeof(on_pflash_op->addr_unlock_c));
	himax_on_parse_assign_cmd(on_flash_data_cmd0, on_pflash_op->data_cmd0, sizeof(on_pflash_op->data_cmd0));
	himax_on_parse_assign_cmd(on_flash_data_cmd1, on_pflash_op->data_cmd1, sizeof(on_pflash_op->data_cmd1));
	himax_on_parse_assign_cmd(on_flash_data_cmd2, on_pflash_op->data_cmd2, sizeof(on_pflash_op->data_cmd2));
	himax_on_parse_assign_cmd(on_flash_data_cmd3, on_pflash_op->data_cmd3, sizeof(on_pflash_op->data_cmd3));
	himax_on_parse_assign_cmd(on_flash_data_lock, on_pflash_op->data_lock, sizeof(on_pflash_op->data_lock));
#endif
#ifdef CORE_SRAM
	/* sram start*/
	himax_on_parse_assign_cmd(on_sram_adr_rawdata_addr, on_psram_op->addr_rawdata_addr, sizeof(on_psram_op->addr_rawdata_addr));
	himax_on_parse_assign_cmd(on_sram_adr_rawdata_end, on_psram_op->addr_rawdata_end, sizeof(on_psram_op->addr_rawdata_end));
	himax_on_parse_assign_cmd(on_sram_cmd_conti, on_psram_op->data_conti, sizeof(on_psram_op->data_conti));
	himax_on_parse_assign_cmd(on_sram_cmd_fin, on_psram_op->data_fin, sizeof(on_psram_op->data_fin));
	himax_on_parse_assign_cmd(on_sram_passwrd_start, on_psram_op->passwrd_start, sizeof(on_psram_op->passwrd_start));
	himax_on_parse_assign_cmd(on_sram_passwrd_end, on_psram_op->passwrd_end, sizeof(on_psram_op->passwrd_end));
	/* sram end*/
#endif
#ifdef CORE_DRIVER
	himax_on_parse_assign_cmd(on_driver_addr_fw_define_int_is_edge, on_pdriver_op->addr_fw_define_int_is_edge, sizeof(on_pdriver_op->addr_fw_define_int_is_edge));
	himax_on_parse_assign_cmd(on_driver_addr_fw_rx_tx_maxpt_num, on_pdriver_op->addr_fw_rx_tx_maxpt_num, sizeof(on_pdriver_op->addr_fw_rx_tx_maxpt_num));
	himax_on_parse_assign_cmd(on_driver_addr_fw_xy_rev_int_edge, on_pdriver_op->addr_fw_xy_rev_int_edge, sizeof(on_pdriver_op->addr_fw_xy_rev_int_edge));
	himax_on_parse_assign_cmd(on_driver_addr_fw_define_x_y_res, on_pdriver_op->addr_fw_define_x_y_res, sizeof(on_pdriver_op->addr_fw_define_x_y_res));
#endif
}
