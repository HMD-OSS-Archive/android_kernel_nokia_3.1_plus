...
extern void touch_scover_write_focalTP(int);
extern int touch_scover_read_focalTP(void);

char *fw_app_bin;
char *tp_ini;
//ZZDC-snow-Touch-ImplementDoubleTap-00+_20170807
int gdouble_tap_enable;
extern int ssover_flag;
                                ...
                                unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		{
            FTS_ERROR("%s fts_ts_resume",__func__);
            fts_ts_resume(&fts_data->client->dev);
		}
        else if (*blank == FB_BLANK_POWERDOWN)
		{
            FTS_ERROR("%s fts_ts_suspend",__func__);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    touch_cb.touch_scover_write = touch_scover_write_focalTP;
    touch_cb.touch_scover_read = touch_scover_read_focalTP;
    touch_cb.touch_double_tap_read = touch_double_tap_read_focalTP; //ZZDC-snow-Touch-ImplementDoubleTap-00+_20170807
    touch_cb.touch_double_tap_write = touch_double_tap_write_focalTP; //ZZDC-snow-Touch-ImplementDoubleTap-00+_20170807
...
static int fts_ts_resume(struct device *dev)
{
    ...
    if (ssover_flag == 1 ) {
        touch_scover_write_focalTP(ssover_flag);
    }
