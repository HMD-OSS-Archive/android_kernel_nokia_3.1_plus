...
enum FIH_BATT_ID {
       ...
       FIH_BATT_ID_SCUD_COSLIGHT = 1, //100k
	   FIH_BATT_ID_MCNAIR_LISHEN = 2, //200k
       FIH_BATT_ID_SCUD_LISHEN = 3,   //300k
};

static char *manufacture;
static char *get_battery_id(void)
{
	switch (gm.battery_id) {
	case FIH_BATT_ID_JIADE_LISHEN:
		manufacture = "JIADE_LISHEN";
		break;
	case FIH_BATT_ID_SCUD_COSLIGHT:
		manufacture = "SCUD_COSLIGHT";
		break;
	case FIH_BATT_ID_MCNAIR_LISHEN:
		manufacture = "MCNAIR_LISHEN";
		break;
	case FIH_BATT_ID_SCUD_LISHEN:
		manufacture = "SCUD_LISHEN";
		break;
	default:
		manufacture = "UNKNOWN";
	}
...
static ssize_t show_battery_manufacture(
	struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	bm_err("%s: %s\n",
...
int BattThermistorConverTemp(int Res)
{
	...
	if (Res >= Fg_Temperature_Table[0].TemperatureR) {
		TBatt_Value = -40;
	} else if (Res <= Fg_Temperature_Table[NTC_TABLE_SIZE-1].TemperatureR) {
		TBatt_Value = Fg_Temperature_Table[NTC_TABLE_SIZE-1].BatteryTemp;
	} else {
		RES1 = Fg_Temperature_Table[0].TemperatureR;
		TMP1 = Fg_Temperature_Table[0].BatteryTemp;
...
/*****************************/
	...
	ret = device_create_file(&(dev->dev), &dev_attr_manufacture); //fid add
