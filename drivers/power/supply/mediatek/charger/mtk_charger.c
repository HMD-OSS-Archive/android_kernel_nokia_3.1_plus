...
static void lcm_on_off_charge_current_work(struct work_struct *work)
{
	struct charger_manager *cm = container_of(work,
						 struct charger_manager,
						 lcm_onoff_charge_curr_work.work);
    _mtk_charger_change_current_setting(cm);
    _wake_up_charger(cm);
}

int charger_manager_set_current_limit_lcm_onoff(struct charger_consumer *consumer,
	int idx, int input_current)
{
	...
	if (info != NULL) {
		...
		pdata->lcm_on_input_current_limit = input_current;
		chr_err("%s: dev:%s idx:%d en:%d\n", __func__,
			dev_name(consumer->dev), idx, input_current);
        #if 1
        schedule_delayed_work(&info->lcm_onoff_charge_curr_work,HZ);
        #else
...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	if (info->battery_temp >= info->data.temp_t4_thres) {
		...
		if ((sw_jeita->sm == TEMP_BELOW_T0)
			...
			sw_jeita->charging = false; //llt add
		}
	} else {
...
static ssize_t show_fih_Input_Current(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct charger_manager *pinfo = dev->driver_data;
...
static DEVICE_ATTR(fih_Input_Current, 0644, show_fih_Input_Current, store_fih_Input_Current);

static ssize_t show_fih_Charging_Control(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct charger_manager *pinfo = dev->driver_data;
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	proc_create_data("en_power_path", 0666, battery_dir,
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	info->chg1_data.lcm_on_input_current_limit = -1;
	info->chg2_data.lcm_on_input_current_limit = -1;
    ...
    INIT_DELAYED_WORK(&info->lcm_onoff_charge_curr_work,lcm_on_off_charge_current_work);
