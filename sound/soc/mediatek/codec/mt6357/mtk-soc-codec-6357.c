...
#endif
#endif
static int g_spk_mode = 3; //rita add
...
{
	pr_info("%s(), enable %d, g_spk_mode %d.\n", __func__, enable, g_spk_mode);
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
		AudDrv_GPIO_EXTAMP_Select(false, g_spk_mode);
		/*udelay(1000); */
		usleep_range(1 * 1000, 2 * 1000);
		AudDrv_GPIO_EXTAMP_Select(true, g_spk_mode);
...
static const char *const spk_mode[] = { "1", "2" ,"3" }; //rita add
...
//rita add
static int apply_spkmode_Set(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{

	int index = ucontrol->value.integer.value[0];
	pr_debug("%s  spkmode =%d\n",
		 __func__, index);
		 
	if (index >= ARRAY_SIZE(spk_mode)) {
		pr_debug("return -EINVAL\n");
		return -EINVAL;
	}
    g_spk_mode = index + 1;
...
static int apply_spkmode_Get(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	if(g_spk_mode > 0 && g_spk_mode <= 3)
	ucontrol->value.integer.value[0] = g_spk_mode - 1;
	return 0;
}
...
static const struct soc_enum Audio_DL_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(apply_n12db_setting),
			    apply_n12db_setting),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(spk_mode), spk_mode), //rita add					
};
static const struct snd_kcontrol_new mt6357_snd_controls[] = {
	...
	SOC_ENUM_EXT("Apply_spk_mode", Audio_DL_Enum[15],
		     apply_spkmode_Get, apply_spkmode_Set), //rita add
};
